# AIM ---------------------------------------------------------------------
# test DE of homeostatic vs MIMs iron at the sc level and pseudobulk.
...
sobj@meta.data %>% pull(cluster_martina) %>% table()
# set the ident
Idents(sobj) <- "cluster_martina"
...
mono.de <- FindMarkers(sobj, ident.1 = "MIMS-iron", ident.2 = "homeo-MG", verbose = FALSE,logfc.threshold = 0)
...
# save the table
saveRDS(mono.de,"../../out/object/141_mono.de.rds")

# test DGE at pseudobulk --------------------------------------------------
# pseudobulk the counts based on donor-condition-celltype
pseudo_sobj <- AggregateExpression(sobj, assays = "RNA", return.seurat = T, group.by = c("dataset", "cluster_martina"),slot = "counts")
...
# save the table
saveRDS(bulk.mono.de,"../../out/object/141_bulk.mono.de.rds")
...
cts_sample_all <- AggregateExpression(object = sobj,
                                      group.by = c("dataset","origin","disease","cluster_martina"),
                                      assays = 'RNA',
                                      slot = "counts",
                                      return.seurat = FALSE)
...
counts <- cts_sample_all$RNA %>%
  as.data.frame() %>%
  select(contains("homeo-MG")|contains("MIMS-iron"))

# 2. generate sample level metadata
LUT_sample <- sobj@meta.data %>%
  group_by(dataset,origin,disease,cluster_martina) %>%
  summarise() %>%
  # harmonyze the naming of the donor_id
  mutate(sample_cell = paste(dataset,origin,disease,cluster_martina,sep = "_")) %>%
  filter(cluster_martina %in% c("homeo-MG","MIMS-iron")) %>%
  ungroup()
...
# perform DESeq2 ----------------------------------------------------------
# build the model
treat <- colData$cluster_martina %>% str_replace(pattern = "-",replacement = ".")
block_donor <- colData$dataset
...
feat_keep <- edgeR::filterByExpr(counts(dds), group = colData$cluster_martina)
...
pdf("../../out/image/141_heatmap_SampleCluster_filterExp.pdf",width = 10,height = 6)
set.seed(1)
hm <- plot_sample_clustering(vds_filter,
                             anno_vars = c("disease","origin","cluster_martina"),
                             distance = "euclidean")
...
plot_vsd <- plotPCA(vds_filter,
                    intgroup = c("cluster_martina","disease","origin")) +
  theme_bw()

plot_vsd$data %>%
  ggplot(aes(x=PC1,y=PC2,col=cluster_martina,shape=origin)) +
  geom_point(size =3,alpha=0.6) +
  # ggrepel::geom_text_repel(show.legend = F)+
  scale_x_continuous(expand = expansion(mult = 0.1))+
  theme_bw() + ylab(plot_vsd$labels[1]) + xlab(plot_vsd$labels[2])
ggsave("../../out/image/141_PCA_pseudobulk_filterExp.pdf",width = 6,height = 4)
...
contrast <- makeContrasts(MIMSIron_vs_HOMEO = treatMIMS.iron,
                          levels = design)

# pull the results table
res_MS <- results(ddsHTSeq_filter, contrast=contrast[,"MIMSIron_vs_HOMEO"],alpha = 0.05)
summary(res_MS)

# add the gene symbols
df_res <-
  list(MIMSIron = res_MS) %>%
  lapply(function(x){
    df <- x %>%
      data.frame() %>%
  ...
  bind_rows(.id = "condVsHOMEO")

# save the table
df_res %>%
  write_tsv("../../out/table/141_DE_pseudobulk_filterExp.tsv")
...
df_res_shr <-
  list(res_MIMSIron_shr = res_MS_shr) %>%
  lapply(function(x){
    df <- x %>%
      data.frame() %>%
      rownames_to_column(var = "symbol") %>%
...
# save the table
df_res_shr %>%
  write_tsv("../../out/table/141_DE_pseudobulk_filterExp_shr.tsv")
...
plot_volcano %>%
  ...
  #     dplyr::filter(padj<0.05),
  #   aes(label = symbol),segment.alpha=0.4) +
  facet_wrap(~condVsHOMEO)+
  theme_bw()+
  theme(strip.background = element_blank())+
  theme(legend.position = "none")
# ggsave("../../out/image/141_vulcano_plot_pseudobulk_filterExp.pdf",width = 10,height = 10)
...
lut <- colData(data) %>%
  data.frame() %>%
  mutate(sample = str_replace_all(sample.id,pattern = "-","."))
...
counts(data,normalized=T)%>%
  ...
  mutate(treat = fct_relevel(cluster_martina,"homeo-MG")) %>%
...
counts(data,normalized=T)%>%
  ...
  geom_boxplot(outlier.shape = NA,linewidth = 1)+
  geom_point(position = position_jitter(width = 0.1),alpha=0.6,size = 3)+
  facet_grid(origin~symbol,scales = "free")+
  scale_y_continuous(trans = "log1p") +
  # scale_y_log10()+
  theme_bw(base_size = 22,base_rect_size = 2)+
  theme(strip.background = element_blank(),
