...
# load a sample data ------------------------------------------------------
test <- readRDS("../../out/object/ManualClean/data.combined_WM_CX_harmonySkipIntegration_AllSoupX_test.rds")
...
list_cell_embeddings <- split(full_data,f = full_data$pathology_class) %>%
  lapply(function(x){
    x %>%
      dplyr::select("UMAP_1","UMAP_2")
  })

list_vars <- split(full_data,f = full_data$pathology_class) %>%
  lapply(function(x){
    x %>%
      # dplyr::select("CD4")
...
dim_names <- colnames(list_cell_embeddings$CX_Ctrl)
...
df_density %>%
  ...
  scale_color_gradientn("sig score",colours = viridis::turbo(10),limits = c(0,0.05),oob = scales::squish)
ggsave(paste0("../../out/image/ManualClean/modules_SENESCENCE/HVG4000/UMAP_score_",x,"_tailored2_nebulosa.pdf"),width = 11,height = 10)
ggsave(paste0("../../out/image/ManualClean/modules_SENESCENCE/HVG4000/UMAP_score_",x,"_tailored2_nebulosa.png"),width = 11,height = 10)
