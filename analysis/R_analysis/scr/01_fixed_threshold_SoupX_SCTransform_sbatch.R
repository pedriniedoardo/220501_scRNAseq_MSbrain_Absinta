...
list_datasc_fixed_norm <- lapply(list_datasc_fixed, function(x){
  ...
  # at this stage I am not regressign out any other confounder variable
  datasc_filter <- SCTransform(datasc_filter,method = "glmGamPoi") 
...
list_datasc_fixed_norm_doublet <- pmap(list(list_datasc_fixed_norm_fix,list_nExp),function(x,y){
  ...
  sweep.res.list <- paramSweep_v3(x, PCs = 1:30, sct = T) 
  ...
  pbmc.seurat.filteres <- doubletFinder_v3(x,PCs = 1:30,pN = 0.25,pK = pK,nExp = nExp.poi.adj,reuse.pANN = F,sct = T)
...
lapply(list_datasc_fixed_norm_doublet, function(x){
  meta <- x@meta.data
  colnames(meta) <- c("orig.ident","nCount_RNA","nFeature_RNA","percent.mt","percent.ribo","mt_bin","official_id","facility","origin","patient","pathology","disease","sex","age","PMI","year_of_extraction","test","not_outlier","nCount_SCT","nFeature_SCT","SCT_snn_res.0.8","seurat_clusters","pANN","DF")
  x@meta.data <- meta
  head(x@meta.data)
  dim(x@meta.data)[1]
})
...
pmap(list(names(list_datasc_fixed_norm_doublet),list_datasc_fixed_norm_doublet),function(x,y){
  ...
  # head(y@meta.data)
  #
  saveRDS(object = y,file = paste0("../../out/object/test_SCTransform/datasc_fix_filter_norm_doublet_",x,"_SoupX_SCTransform.rds"))
})
...
# save the total meta
meta_total_doublet %>%
  write_tsv("../../out/table/test_SCTransform/meta_datasc_fix_filter_norm_doublet_SoupX_SCTransform.tsv")
...
pmap(list(names(list_datasc_fixed_norm_doublet),list_datasc_fixed_norm_doublet),function(x,y){
  ...
  saveRDS(object = datasc_filter,file = paste0("../../out/object/test_SCTransform/datasc_fix_filter_norm_doublet_Singlet_",x,"_SoupX_SCTransform.rds"))
})
