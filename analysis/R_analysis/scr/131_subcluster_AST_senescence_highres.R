...
# ggsave("../../out/image/131_UMAP_senescence_ASTsubset.pdf",width = 25,height = 20)
...
df_senescence <- data.combined@meta.data %>%
  dplyr::select(RNA_snn_res.0.5,contains("scoreSen_")) %>%
...
pdf("../../out/image/131_heatmap_senescence_res0.5_ASTsubset.pdf",width = 10,height = 5)
draw(hm01,heatmap_legend_side = "left",padding = unit(c(2, 2, 2, 80), "mm"))
dev.off()

# save the table with the values
df_senescence %>%
  group_by(signature) %>%
  mutate(scaled_avg_score = (avg_score - mean(avg_score))/sd(avg_score)) %>%
  ungroup() %>%
  write_tsv("../../out/table/131_heatmap_table_senescence_res0.5_ASTsubset.tsv")
