...
scobj <- readRDS("../../out/object/data.combined_fix_filter_norm_doublet_integrated_SoupX_harmony.rds")

# save the current meta
scobj@meta.data %>% 
  data.frame() %>% 
  rownames_to_column() %>% 
  write_tsv("../../out/table/meta_data.combined_fix_filter_norm_doublet_integrated_SoupX_harmony.tsv")

# plotting ----------------------------------------------------------------
# confirm the identity of the object
DimPlot(scobj,label = T,raster = T)
ggsave("../../out/image/UMAP_data.combined_fix_filter_norm_doublet_integrated_SoupX_harmony.pdf",width = 8,height = 7)

DimPlot(scobj,label = T,raster = T,split.by = "official_id",ncol = 5)
ggsave("../../out/image/UMAP_data.combined_fix_filter_norm_doublet_integrated_SoupX_harmony_splitSample.pdf",width = 26,height = 20)

# plot some metadata
DimPlot(scobj,label = T,raster = T,group.by = "nCount_RNA.harmony")
ggsave("../../out/image/UMAP_data.combined_fix_filter_norm_doublet_integrated_SoupX_harmony.pdf",width = 8,height = 7)
...
df_summary <- scobj@meta.data %>% 
  ...
  ungroup() %>% 
  mutate(prop = n/tot)
write_tsv(df_summary,"../../out/table/df_summary_SoupX_harmony.tsv")
...
write_tsv(df_summary2,"../../out/table/df_summary_SoupX_harmony_testRemove7.tsv")
...
ggsave("../../out/image/plot_summary_SoupX_harmony.pdf",width = 8,height = 4)
...
test %>% 
  dplyr::filter(seurat_clusters == "14") %>% 
  summarise(n = n())

test %>% 
  dplyr::filter(percent.mt.harmony < 5) %>%
  dplyr::filter(seurat_clusters == "2") %>% 
  summarise(n = n())
...
test %>% 
  # dplyr::filter(nFeature_RNA.harmony > 500) %>%
  dplyr::filter(seurat_clusters == "7") %>% 
  summarise(n = n())
...
test %>% 
  ...
  ggplot(aes(x=seurat_clusters,y=value))+geom_violin()+geom_boxplot(width=0.1,outlier.shape = NA)+facet_wrap(~variable,scales = "free")+theme_bw()+theme(strip.background = element_blank())
...
test_long2
ggsave("../../out/image/Dotplot_data.combined_fix_filter_norm_doublet_integrated_SoupX_harmony.pdf",width = 23,height = 4)
