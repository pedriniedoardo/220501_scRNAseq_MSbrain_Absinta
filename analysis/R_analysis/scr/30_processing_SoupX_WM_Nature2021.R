...
# READ IN DATA ------------------------------------------------------------
# # read in the dataset with the original metadata
# WM_original <- readRDS("../../data/WM_data/WM_data_Absinta_Nature2021/all20_integrated_clean_metadata.rds")
# 
# # DimPlot(WM_original)
# 
# # read in the metadata of the original object
# meta_WM_original<- read_csv("../../data/LUT_WM_Nature2021.csv") %>%
#   mutate(sample_seurat = as.character(sample_seurat))
# 
# meta_WM_ref <- WM_original@meta.data %>%
#   data.frame() %>%
#   left_join(meta_WM_original,by = c("sample" = "sample_seurat"))
# 
# # add the missing sample id to the original dataset
# WM_original$sample_id <- meta_WM_ref$sample_id
# 
# # split the dataset into samples
# list_WM_ref <- SplitObject(WM_original,split.by = "sample_id")
# 
# # extract the original metadata from each object
# list_meta_WM_ref <- lapply(list_WM_ref, function(x){
#   x@meta.data %>%
#     data.frame() %>%
#     rownames_to_column("barcodes")
# })
# 
# # save the list of the metadata
# saveRDS(list_meta_WM_ref,file = "../../out/object/list_meta_WM_ref.rds")

# read in the list of all the metadata used as reference
list_meta_WM_ref <- readRDS(file = "../../out/object/list_meta_WM_ref.rds")

# read in the new data
id_sample <- dir("../../data/SoupX_default_WM/") %>% 
  str_subset(pattern = "s25|s26|s27|s28|s29|s30|s31|s32",negate = T)
...
list_datasc <- lapply(id_sample,function(x){
  ...
  meta_ref <- list_meta_WM_ref_fix[[x]] %>% 
    ...
    dplyr::select(barcode_id,sample,disease,pathology,batch,patient,patient2,plaque,sex,age,pmi,seurat_clusters_ref=seurat_clusters,sample_id)
  ...
  meta_test <- datasc@meta.data %>% 
    ...
    left_join(meta_ref,by = "barcode_id") %>% 
