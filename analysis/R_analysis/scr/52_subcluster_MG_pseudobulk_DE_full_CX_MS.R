...
counts_MG <- bind_cols(counts_MG_Ctrl,counts_MG_Mye) %>%
  select(contains("CX_"))
...
saveRDS(counts_MG,file = "../../out/object/counts_MG_CX.rds")
saveRDS(colData_MG,file = "../../out/object/colData_MG_CX.rds")
...
# save the disign
saveRDS(design_MG,"../../out/object/design_MG_CX.rds")
...
pdf("../../out/image/heatmap_SampleCluster_pseudobulk_MG_CX.pdf",width = 10,height = 6)
set.seed(3)
...
plot_vsd_MG$data %>%
  ...
  scale_x_continuous(expand = expansion(mult = 0.1))+
  theme_bw() + ylab(plot_vsd_MG$labels[1]) + xlab(plot_vsd_MG$labels[2])
ggsave("../../out/image/PCA_pseudobulk_MG_CX.pdf",width = 6,height = 4)
...
pdf("../../out/image/heatmap_cellType_pseudobulk_MG_CX.pdf",width = 7,height = 5)
...
pdf("../../out/image/heatmap_cellType_pseudobulk_MG_CX2.pdf",width = 7,height = 5)
...
saveRDS(ddsHTSeq_MG_filter,"../../out/object/ddsHTSeq_pseudobulk_MG_CX.rds")
...
# save the table
df_res %>%
  write_tsv("../../out/table/DE_pseudobulk_MG_CX.tsv")
...
df_res_shr <-
  list(MG.CX.MS_shr = res_MG.MS_shr) %>%
  lapply(function(x){
    df <- x %>%
      data.frame() %>%
      rownames_to_column(var = "symbol") %>%
...
# save the table
df_res_shr %>%
  write_tsv("../../out/table/DE_pseudobulk_MG_CX_shr.tsv")
...
ggsave("../../out/image/histogram_pvalue_pseudobulk_MG_CX.pdf",width = 6,height = 4)
...
ggsave("../../out/image/vulcano_plot_pseudobulk_MG_CX.pdf",width = 10,height = 10)
...
pdf("../../out/image/heatmap_DEG_plot_pseudobulk_MG_CX_shr.pdf",width = 7,height = 14)
...
ht2_shr_MG2 <- Heatmap(mat2_shr_MG2, show_column_names = T,raster_by_magick = T,show_row_dend = F, use_raster = T,
                      ...
                      column_title = "MG_shr",
                      # row_names_gp = gpar(fontsize = 3),
                      top_annotation = column_ha_shr_MG2,show_row_names = T,
                      # cluster_rows = F,
                      # right_annotation = row_ha,
                      # row_split = rep(c(1,2,3,4),c(2,3,4,7))
                      
)
pdf("../../out/image/heatmap_DEG_plot_pseudobulk_MG_CX.pdf",width = 7,height = 14)
draw(ht2_shr_MG2,heatmap_legend_side = "left",annotation_legend_side = "left",padding = unit(c(30,2,2, 2), "mm"))
dev.off()
