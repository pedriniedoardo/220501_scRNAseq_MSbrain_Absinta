...
# in this case use the cells from cluster 0,5 and 9 from res 0.4 which are the confident endothelial cells
# Idents(data.combined) <- "RNA_snn_res.0.4"
scobj_subset <- subset(data.combined,subset = RNA_snn_res.0.4 %in% c(0,5,9))
...
meta_summary <- scobj_subset@meta.data %>%
  group_by(orig.ident,origin,disease,age,pathology_class) %>%
  summarise(avg_percent.mt = mean(percent.mt), avg_percent.ribo = mean(percent.ribo))

# wrangling ---------------------------------------------------------------
# explore the full meta
scobj_subset@meta.data %>%
  group_by(orig.ident,origin,disease) %>%
  summarise(n = n(),.groups = "drop") %>%
  mutate(test = paste(origin,disease)) %>%
  dplyr::select(-c(origin,disease)) %>%
...
cts_sample_all <- AggregateExpression(object = scobj_subset,
                                      group.by = c("origin","disease","orig.ident","sex"),
                                      assays = 'RNA',
                                      slot = "counts",
                                      return.seurat = FALSE)
...
meta_reads <- colSums(counts) %>% data.frame(tot_reads = .) %>% mutate(M_reads = tot_reads/10^6) %>% rownames_to_column("sample") %>%
  separate(sample,into = c("origin","disease","orig.ident","sex"),sep = "_") %>%
  dplyr::select(-c(tot_reads,sex,disease,origin)) %>%
  mutate(orig.ident = str_replace_all(orig.ident,pattern = "-","_"))

# 2. generate sample level metadata
LUT_sample <- scobj_subset@meta.data %>%
  group_by(orig.ident,sex,disease,origin) %>%
  summarise() %>%
  # match the orig.ident with the one in the LUT
  mutate(orig.ident_fix = str_replace_all(orig.ident,pattern = "_",replacement = "-")) %>%
  # harmonyze the naming of the donor_id
  # mutate(sample_disease = paste(orig.ident_fix,disease,sep = "_")) %>%
  mutate(sample_disease_origin = paste(origin,disease,orig.ident_fix,sex,sep = "_")) %>%
  ungroup() %>%
  # add the summary info
  left_join(meta_summary,by = c("orig.ident","origin","disease")) %>%
  # add total number of reads per sample
  left_join(meta_reads,by = "orig.ident")
...
saveRDS(counts,file = paste0("../../out/object/145_counts_mono_pBulk_","ENDO",".rds"))
saveRDS(colData,file = paste0("../../out/object/145_colData_mono_pBulk_","ENDO",".rds"))

# perform DESeq2 ----------------------------------------------------------
# build the model
treat <- colData$disease
# gender <- colData$sex
origin <- colData$origin

# design <- model.matrix(~ block_donor + treat)
# design <- model.matrix(~ gender + origin + treat)
design <- model.matrix(~ origin + treat)
colnames(design)[1] <- c("intercept")

# save the disign
saveRDS(design,paste0("../../out/object/145_design_mono_pBulk","ENDO",".rds"))
...
ggsave(paste0("../../out/image/145_pBulk_totReads_","ENDO",".pdf"),width = 10,height = 6)
...
pdf(paste0("../../out/image/145_heatmap_SampleCluster_filterExp_","ENDO",".pdf"),width = 12,height = 6)
set.seed(21)
hm <- plot_sample_clustering(vds_filter,
                             anno_vars = c("disease","origin","sex","M_reads"),
                             distance = "euclidean")
...
plot_vsd <- plotPCA(vds_filter,
                    intgroup = c("disease","sex","origin","age","avg_percent.mt","avg_percent.ribo","M_reads")) +
  theme_bw()

plot_vsd$data %>%
  separate(name,into = c("origin","disease","orig.ident","sex"),sep = "_",remove = F) %>%
  ggplot(aes(x=PC1,y=PC2,col=origin)) +
  geom_point(size =3,alpha=0.6) +
  # ggrepel::geom_text_repel(show.legend = F)+
  scale_x_continuous(expand = expansion(mult = 0.1))+
  theme_bw() + ylab(plot_vsd$labels[1]) + xlab(plot_vsd$labels[2])
ggsave(paste0("../../out/image/145_PCA_pseudobulk_filterExp_","ENDO",".pdf"),width = 12,height = 10)

plot_vsd$data %>%
  separate(name,into = c("origin","disease","orig.ident","sex"),sep = "_",remove = F) %>%
  ggplot(aes(x=PC1,y=PC2,col=disease,label = orig.ident)) +
  geom_point(size =3,alpha=0.6) +
  ggrepel::geom_text_repel(show.legend = F)+
  scale_x_continuous(expand = expansion(mult = 0.1))+
  theme_bw() + ylab(plot_vsd$labels[1]) + xlab(plot_vsd$labels[2])
ggsave(paste0("../../out/image/145_PCA_pseudobulk_filterExp2_","ENDO",".pdf"),width = 12,height = 10)
...
left_join(plot_vsd$data %>% dplyr::select(-c("PC1","PC2")),test,by = c("name"="sample")) %>%
  ...
  ggpairs(columns = 10:19,ggplot2::aes(colour=disease),upper = "blank") +
  theme_bw()+theme(strip.background = element_blank(),axis.text.x = element_text(hjust = 1,angle = 45))
ggsave(paste0("../../out/image/145_PCA_pseudobulk_panel_","ENDO",".pdf"),width = 20,height = 20)
...
test_df1 <- test2 |> 
  dplyr::select(name,disease,sex,origin) |> 
...
left_join(test_df1,test_df2,by=c("name")) |> 
  ...
  theme_bw()+theme(strip.background = element_blank(),axis.text.x = element_text(hjust = 1,angle = 45))
ggsave(paste0("../../out/image/145_panel_metadata_PC_pbulk_cat_","ENDO",".pdf"),width = 20,height = 10)

# continuos variables
# test2 <- left_join(plot_vsd$data %>% dplyr::select(-c("PC1","PC2")),test,by = c("name"="sample"))

test_df3 <- test2 |> 
  dplyr::select(name,age,avg_percent.mt,avg_percent.ribo,M_reads) |> 
  # dplyr::rename(approx.time = Approx.Time.between.collection.and.processing) |> 
  pivot_longer(names_to = "var_1",values_to = "value_1",-c(name))

test <- left_join(test_df3,test_df2,by=c("name")) |> 
  mutate(var_2 = str_remove(var_2,"PC") %>% str_pad(side = "left",width = 2,pad = "0") %>% paste0("PC",.)) |> 
  mutate(comparison = paste0(var_1,"_vs_",var_2))

test %>%
  filter(var_1 == "M_reads") %>%
  ggplot(aes(x=value_1,y = value_2)) +
  facet_wrap(~comparison,scales = "free",ncol=10) +
  # facet_grid(var_1~var_2,scales = "free_x",drop = T) +
  # geom_boxplot(outlier.shape = NA)+
...
left_join(test_df3,test_df2,by=c("name")) |> 
  ...
  theme_bw()+theme(strip.background = element_blank(),axis.text.x = element_text(hjust = 1,angle = 45))
ggsave(paste0("../../out/image/145_panel_metadata_PC_pbulk_cont_","ENDO",".pdf"),width = 20,height = 10)

# cell composition --------------------------------------------------------
# load the full dataset
# scobj <- readRDS("../../out/object/129_MG_subcluster_HarmonySample_martinaCluster.rds")
DimPlot(data.combined,group.by = "RNA_snn_res.0.4")

# check the relative proportion of different cell types from different sample
sample_prop_wide <- data.combined@meta.data %>%
  group_by(orig.ident,sex,origin,disease,RNA_snn_res.0.4) %>%
  summarise(n = n(),.groups = "drop") %>%
  # match the orig.ident with the one in the LUT
  mutate(orig.ident_fix = str_replace_all(orig.ident,pattern = "_",replacement = "-")) %>%
  mutate(sample_disease_origin = paste(origin,disease,orig.ident_fix,sex,sep = "_")) %>%
  # ensure all the combinations are presents
  complete(RNA_snn_res.0.4,sample_disease_origin, fill = list(n = 0)) %>%
  group_by(sample_disease_origin) %>%
  mutate(tot = sum(n)) %>%
  mutate(prop = n/tot) %>%
  # make it wider
  dplyr::select(RNA_snn_res.0.4,sample_disease_origin,prop) %>%
...
column_meta_sample_prop <- HeatmapAnnotation(diagnosis = meta_sample_prop$disease,
                                             origin = meta_sample_prop$origin,
                                             col = list(diagnosis = c("CTRL" = "blue",
                                                                      "MS" = "red"),
                                                        origin = c("cortex" = "gray",
                                                                     "wm" = "black")))
...
# pdf(paste0("../../out/image/analysis_R44/145_heatmap_cellType_pseudobulk_","ENDO",".pdf"),width = 10,height = 8)
...
sample_prop_wide2 <- data.combined@meta.data %>%
  ...
  # make it wider
  dplyr::select(RNA_snn_res.0.4,sample_disease_origin,scale_prop) %>%
  pivot_wider(names_from = sample_disease_origin,values_from = scale_prop) %>%
  # pivot_wider(names_from = stim_donor,values_from = prop,values_fill = 0) %>%
  column_to_rownames("RNA_snn_res.0.4") %>%
  # select only the samples in the dataset
  dplyr::select(colData$sample.id)
...
# pdf(paste0("../../out/image/analysis_R44/145_heatmap_cellType_pseudobulk2_","ENDO",".pdf"),width = 10,height = 8)
...
saveRDS(ddsHTSeq_filter,paste0("../../out/object/145_ddsHTSeq_pseudobulk_filterExp_","ENDO",".rds"))
...
df_res %>%
  mutate(cluster = "ENDO") %>%
  write_tsv(paste0("../../out/table/145_DE_pseudobulk_filterExp_","ENDO",".tsv"))
...
df_res_shr %>%
  mutate(cluster = "ENDO") %>%
  write_tsv(paste0("../../out/table/145_DE_pseudobulk_filterExp_shr_","ENDO",".tsv"))
...
ggsave(paste0("../../out/image/145_histogram_pvalue_pseudobulk_filterExp_","ENDO",".pdf"),width = 6,height = 4)
...
ggsave(paste0("../../out/image/145_vulcano_plot_pseudobulk_filterExp_","ENDO",".pdf"),width = 10,height = 10)
...
plot_volcano_shr %>%
  ...
  theme(strip.background = element_blank())+
  theme(legend.position = "none")
ggsave(paste0("../../out/image/145_vulcano_plot_pseudobulk_filterExp_shr_","ENDO",".pdf"),width = 10,height = 10)
...
meta_sample <- data.frame(colname = colnames(mat2_shr)) %>%
  left_join(colData,by=c("colname"="sample.id")) %>%
  mutate(origin = factor(origin,levels = c("cortex","wm")))
...
pdf(paste0("../../out/image/145_heatmap_DEG_plot_pseudobulk_filterExp_shr_","ENDO",".pdf"),width = 10,height = 8)
draw(ht2_shr,heatmap_legend_side = "left",annotation_legend_side = "left",padding = unit(c(30,2,2, 2), "mm"))
dev.off()
...
# PLOT DISPERSION ---------------------------------------------------------
pdf(paste0("../../out/image/145_dispersion_plot_pseudobulk_filterExp_","ENDO",".pdf"),width = 5,height = 5)
plotDispEsts(ddsHTSeq_filter)
dev.off()

# compare gene lists ------------------------------------------------------
# load the table of DE from Eliana
df_Eliana <- readRDS("../../out/object/143_custom_signatures_eliana_endo.rds") %>%
  bind_rows()

# load the DE analysis from the subset dataset
df_res_shr <- read_tsv(paste0("../../out/table/145_DE_pseudobulk_filterExp_shr_","ENDO",".tsv"))

df_Eliana %>%
  left_join(df_res_shr,by = c("Genes" = "symbol")) %>%
  mutate(test = case_when(padj < 0.05~"sig",
                          T~"non_sig")) %>%
  ggplot(aes(x=logFC,y=log2FoldChange)) +
  geom_point(aes(size= -log(padj),col = test),alpha=0.6) +
  ...
  xlab("logFC Eliana Bulk") +
  ylab("logFC Absinta ENDO sc")+
  scale_color_manual(values = c("black","red"))
ggsave("../../out/image/145_scatter_logFC_bulk_vs_sc.pdf",width = 17,height = 16)
