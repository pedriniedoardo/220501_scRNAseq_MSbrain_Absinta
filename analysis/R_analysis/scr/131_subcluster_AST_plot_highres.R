...
# read in the data --------------------------------------------------------
data.combined <- readRDS("../../out/object/131_AST_subcluster_HarmonySample.rds")
...
#                    prefix = "RNA_snn_res.")
# ggsave("../../out/image/131_UMAPCluster_tree_AST_subcluster.pdf",width = 10,height = 10)

# general UMAP with new clustering
# DimPlot(data.combined, reduction = "umap",group.by = "RNA_snn_res.0.1",label = T,raster = T)
# ggsave("../../out/image/131_UMAPCluster_res0.1_AST_subcluster.pdf",width = 4,height = 3)

DimPlot(data.combined, reduction = "umap",group.by = "RNA_snn_res.0.5",label = T,raster = T)
ggsave("../../out/image/131_UMAPCluster_res0.5_AST_subcluster.pdf",width = 5,height = 4)

DimPlot(data.combined, reduction = "umap",group.by = "RNA_snn_res.0.5",split.by = "origin",label = T,raster = T,ncol=4)
ggsave("../../out/image/131_UMAPCluster_splitTreat_res0.5_AST_subcluster.pdf",width = 12,height = 6)
...
# ggsave("../../out/image/131_UMAPCluster_resolutions_AST_subcluster.pdf",width = 25,height = 15)
...
test_long01 <- DotPlot(data.combined,
  ...
  RotatedAxis() +
  labs(title = "RNA_snn_res.0.5")+
  theme(strip.text = element_text(angle = 90))
ggsave(plot=test_long01,"../../out/image/131_DotplotLong_res0.5_AST_subcluster.pdf",width = 30,height = 5)
...
# ggsave("../../out/image/131_UMAPCluster_short_AST_subcluster.pdf",width = 22,height = 12)
...
# ggsave("../../out/image/131_UMAPCluster_long_AST_subcluster.pdf",width = 22,height = 12)

# same as above but as violin plot
list_plot <- lapply(df_rename_long$rename, function(x){ 
  test <- VlnPlot(object = data.combined,features = x, group.by = "RNA_snn_res.0.5",raster = T)
  return(test)
})
...
df_signatureAbsinta <- data.combined@meta.data %>%
  dplyr::select(RNA_snn_res.0.5,contains("scoreLong_")) %>%
  pivot_longer(names_to = "signature",values_to = "score",-RNA_snn_res.0.5) %>%
  group_by(RNA_snn_res.0.5,signature) %>%
  summarise(avg_score = mean(score),
            med_score = median(score)) %>%
  mutate(cluster_id = paste0("clu_",RNA_snn_res.0.5,"_res0.5")) %>%
  ungroup()
...
pdf("../../out/image/131_heatmap_signatureLong_res0.5_ASTsubset.pdf",width = 10,height = 4)
...
pdf("../../out/image/131_heatmap_signatureLong_res0.5_ASTsubset2.pdf",width = 8,height = 5)
...
sobj_total_h.markers %>%
  write_tsv("../../out/table/131_FindAllMarkers_HarmonySample_res0.5_AST_subcluster.tsv")

# pick the top 100 markers per cluster
sobj_total_h.markers %>%
  group_by(cluster) %>%
  dplyr::slice(1:100) %>%
  write_tsv("../../out/table/131_FindAllMarkers_HarmonySample_res0.5_AST_subcluster_top100.tsv")
...
sobj_total_h.markers %>%
  ...
  write_tsv("../../out/table/131_FindAllMarkers_HarmonySample_res0.5_AST_subcluster_top100_noRIBOandMT.tsv")
...
ggsave("../../out/image/131_Ditto_HarmonySample_res0.5_AST_subcluster.pdf",width = 15,height = 5)

# plot the proportions
df_summary <- data.combined@meta.data %>%
  group_by(dataset,pathology_class,origin,RNA_snn_res.0.5) %>%
  summarise(n = n()) %>%
  ungroup() %>%
...
df_summary %>%
  ...
  facet_wrap(~RNA_snn_res.0.5,ncol = 1,scales = "free")+
  scale_y_sqrt()
# ggsave("../../out/image/131_plot_clusterProp_res0.1_AST_subcluster.pdf",height = 20,width = 4)
...
out_diagnosis <- propeller(clusters = meta_test$RNA_snn_res.0.5,
                           sample = paste0(meta_test$dataset),
                           group = meta_test$pathology_class)

# out_diagnosis %>%
#   rownames_to_column("RNA_snn_res.0.1") %>%
#   write_tsv("../../out/table/131_propeller_res0.1_AST_subcluster.tsv")

# plotting diagnosis ------------------------------------------------------
# default plot
speckle::plotCellTypeProps(x = data.combined,
                           clusters = data.combined$RNA_snn_res.0.5,
...
#   write_tsv("../../out/table/131_df_summary_diagnosis_res0.1_AST_subcluster.tsv")
...
df_summary_diagnosis %>%
  ggplot() +
  geom_boxplot(aes(x=RNA_snn_res.0.5,y=prop,color=pathology_class),outlier.shape = NA) +
  geom_point(aes(x=RNA_snn_res.0.5,y=prop,color=pathology_class),position = position_jitterdodge(jitter.width = 0.1,dodge.width = 0.8),alpha=0.7) +
  theme_cowplot()+
