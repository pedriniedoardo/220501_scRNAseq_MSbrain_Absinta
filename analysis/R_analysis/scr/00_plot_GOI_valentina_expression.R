...
ggsave("../../out/image/00_dotplot_annotationConfident_Valentina_brain_expressionAvg_cellType.pdf",width = 6,height = 6)
...
df_avg |>
  ...
  theme(strip.background = element_blank(),
        panel.border = element_rect(colour = "black", fill = NA))+
  facet_wrap(gene~expertAnno.l1,scales = "free",ncol = 9)
# scale_fill_viridis_c(option = "plasma",name="log10 number \nof cells")
...
df_avg %>%
  ...
  theme(strip.background = element_blank(),
        panel.border = element_rect(colour = "black", fill = NA))+
  facet_wrap(gene~expertAnno.l1,nrow=9)
# scale_fill_viridis_c(option = "plasma",name="log10 number \nof cells")
