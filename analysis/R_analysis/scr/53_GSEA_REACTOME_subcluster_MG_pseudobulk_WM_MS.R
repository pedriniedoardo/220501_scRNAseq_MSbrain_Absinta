...
df_tables_GSEA_all %>%
  write_tsv("../../out/table/GSEA_REACTOME_DE_pseudobulk_MG_MS_shr.tsv")
...
# save the table
df_tables_GSEA_all_non_redundant %>%
  write_tsv("../../out/table/GSEA_REACTOME_DE_pseudobulk_MG_MS_shr_nonredundant.tsv")
...
ggsave("../../out/image/GSEA_REACTOME_DE_pseudobulk_MG_MS_shr_nonredundant.pdf",width = 15,height = 10)
...
df_tables_GSEA_all %>%
  ...
  geom_hline(yintercept = -log10(0.05),linetype="dashed",col="gray")+
  geom_text_repel(size = 2,box.padding = 0.5,segment.alpha = 0.6,max.overlaps = 10)
ggsave("../../out/image/GSEA_REACTOME_DE_pseudobulk_MG_MS_shr.pdf",width = 15,height = 10)
...
df_tables_GSEA_all %>%
  data.frame() %>%
  dplyr::filter(str_detect(pathway,pattern = "TNF"))
...
df_tables_GSEA_all_non_redundant %>%
  data.frame() %>%
  filter(pathway == "REACTOME_TNF_SIGNALING")
