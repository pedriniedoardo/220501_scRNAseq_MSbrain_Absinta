# AIM ---------------------------------------------------------------------
# compare the subclustering of VAS with the original object
...
sobj_VAS <- readRDS("../../out/object/130_VAS_subcluster_HarmonySample.rds")
DimPlot(sobj_VAS,group.by = "RNA_snn_res.0.1",raster = T)

# wrangling ---------------------------------------------------------------
# add the rename cluster Id from the res 0.3 to the original object
meta_test_subcluster <- sobj_VAS@meta.data %>%
  rownames_to_column("barcode") %>%
  dplyr::select(clu_VAS_sub = RNA_snn_res.0.1,barcode)
...
sobj_tot@meta.data$test_cluster <- sobj_tot@meta.data %>%
  ...
  mutate(test_cluster = case_when(is.na(clu_VAS_sub) ~ expertAnno.l1,
...
#
test_long_subVAS <- DotPlot(sobj_VAS,
                           features = shortlist_features_list_long,
                           dot.scale = 8,
                           cluster.idents = T,
                           group.by = "RNA_snn_res.0.1") +
  RotatedAxis() +
  labs(title = "VAS subset res 0.1")+
  theme(strip.text = element_text(angle = 90))

# generate the ggplot object
# force the order suggested by matrina
df_test_subVAS <- lapply(shortlist_features_list_long,function(x){
  test_long_subVAS$data %>% 
    filter(features.plot %in% x)
}) %>% 
  bind_rows(.id = "cell_type")

# plot mapping radius
test_long_subMG_alt <- df_test_subVAS %>%
  # force the order
  # mutate(id = factor(id,levels = c("IMM","LYM","INH NEU","EXC NEU","AST","OLIGO","OPC","VAS","EPENDYMA"))) %>% 
  mutate(cell_type = factor(cell_type,levels = c("IMMUNE","B_CELLS","T_CELLS","NEURONS","ASTRO","OLIGOLINEAGE","ENDO","PERICYTE","SCHWANN","EPENDYMA","STROMAL"))) %>% 
  ggplot(aes(x = features.plot,y = id)) +
...
test_long_run032 <- DotPlot(sobj_tot,
  ...
  RotatedAxis() +
  labs(title = "WM+CX with VAS subcluster")+
  theme(strip.text = element_text(angle = 90))
...
test_long_run032_alt <- df_test_run032 %>%
  # force the order
  mutate(id = factor(id,levels = c(0:7,"IMM","LYM","INH NEU","EXC NEU","AST","OLIGO","OPC","EPENDYMA"))) %>%
  mutate(cell_type = factor(cell_type,levels = c("IMMUNE","B_CELLS","T_CELLS","NEURONS","ASTRO","OLIGOLINEAGE","ENDO","PERICYTE","SCHWANN","EPENDYMA","STROMAL"))) %>% 
  ggplot(aes(x = features.plot,y = id)) +
  geom_point(aes(size = pct.exp, col = avg.exp.scaled))+
  scale_radius(range = c(0, 8)) +
...
ggsave(plot = (test_long_run03_alt / test_long_subMG_alt / test_long_run032_alt),"../../out/image/130_DotplotLong_VAS_subcluster_test.pdf",width = 20,height = 18)
