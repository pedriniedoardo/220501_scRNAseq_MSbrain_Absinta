...
list_genes_UP <- read_tsv("../../out/table/DE_pseudobulk_OLIGO_ctrl_refCX_shr.tsv") %>%
...
list_genes_DOWN <- read_tsv("../../out/table/DE_pseudobulk_OLIGO_ctrl_refCX_shr.tsv") %>%
  filter(padj<0.05 & log2FoldChange < (-1)&!is.na(symbol)) %>%
  split(f = .$conditionVsCX) %>%
  map(function(x){
    x %>%
...
list_enrichr_UP %>%
  write_tsv("../../out/table/enrichR_DE_pseudobulk_OLIGO_ctrl_refCX_UP_shr.tsv")
...
list_enrichr_DOWN %>%
  write_tsv("../../out/table/enrichR_DE_pseudobulk_OLIGO_ctrl_refCX_DOWN_shr.tsv")
...
ggsave("../../out/image/enrichR_DE_pseudobulk_OLIGO_ctrl_refCX_UP_shr.pdf",width = 6,height = 12,limitsize = FALSE)
...
ggsave("../../out/image/enrichR_DE_pseudobulk_OLIGO_ctrl_refCX_DOWN_shr.pdf",width = 6,height = 12,limitsize = FALSE)
