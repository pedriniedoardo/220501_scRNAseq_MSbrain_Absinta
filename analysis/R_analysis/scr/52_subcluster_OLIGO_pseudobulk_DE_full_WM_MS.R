...
identical(rownames(counts_OLIGO_Ctrl),rownames(counts_OLIGO_NAWM))
# identical(rownames(counts_OLIGO_Mye),rownames(counts_OLIGO_NAWM))

# merge the three tables
# filter only for the CX samples
counts_OLIGO <- bind_cols(counts_OLIGO_Ctrl,counts_OLIGO_NAWM) %>%
  select(contains("WM_"))
...
saveRDS(counts_OLIGO,file = "../../out/object/counts_OLIGO_full_WM.rds")
saveRDS(colData_OLIGO,file = "../../out/object/colData_OLIGO_full_WM.rds")
...
# save the disign
saveRDS(design_OLIGO,"../../out/object/design_OLIGO_full_WM.rds")
...
pdf("../../out/image/heatmap_SampleCluster_pseudobulk_OLIGO_full_WM.pdf",width = 10,height = 6)
set.seed(3)
...
plot_vsd_OLIGO$data %>%
  ...
  scale_x_continuous(expand = expansion(mult = 0.1))+
  theme_bw() + ylab(plot_vsd_OLIGO$labels[1]) + xlab(plot_vsd_OLIGO$labels[2])
ggsave("../../out/image/PCA_pseudobulk_OLIGO_full_WM.pdf",width = 6,height = 4)
...
pdf("../../out/image/heatmap_cellType_pseudobulk_OLIGO_full_WM.pdf",width = 7,height = 5)
...
pdf("../../out/image/heatmap_cellType_pseudobulk_OLIGO_full_WM2.pdf",width = 7,height = 5)
...
saveRDS(ddsHTSeq_OLIGO_filter,"../../out/object/ddsHTSeq_pseudobulk_OLIGO_full_WM.rds")
...
# save the table
df_res %>%
  write_tsv("../../out/table/DE_pseudobulk_OLIGO_full_WM.tsv")
...
df_res_shr <-
  list(OLIGO.WM.MS_shr = res_OLIGO.MS_shr) %>%
  lapply(function(x){
    df <- x %>%
      data.frame() %>%
      rownames_to_column(var = "symbol") %>%
...
# save the table
df_res_shr %>%
  write_tsv("../../out/table/DE_pseudobulk_OLIGO_full_WM_shr.tsv")
...
ggsave("../../out/image/histogram_pvalue_pseudobulk_OLIGO_full_WM.pdf",width = 6,height = 4)
...
plot_volcano %>%
  ...
  ggrepel::geom_text_repel(
    data = plot_volcano %>% group_by(conditionVsCTRL) %>% arrange(padj) %>% dplyr::slice(1:60),
    aes(label = symbol),segment.alpha=0.4) +
  facet_wrap(~conditionVsCTRL)+
  theme_bw()+
  theme(strip.background = element_blank())+
  theme(legend.position = "none")
ggsave("../../out/image/vulcano_plot_pseudobulk_OLIGO_full_WM.pdf",width = 10,height = 10)
...
pdf("../../out/image/heatmap_DEG_plot_pseudobulk_OLIGO_full_WM_shr.pdf",width = 7,height = 14)
...
pdf("../../out/image/heatmap_DEG_plot_pseudobulk_OLIGO_full_WM.pdf",width = 7,height = 14)
draw(ht2_shr_OLIGO2,heatmap_legend_side = "left",annotation_legend_side = "left",padding = unit(c(30,2,2, 2), "mm"))
dev.off()
