...
results <- read_tsv("../../out/table/DE_pseudobulk_OPC_ctrl_refCX_shr.tsv") %>%
...
df_tables_GSEA_all %>%
  write_tsv("../../out/table/GSEA_REACTOME_DE_pseudobulk_OPC_ctrl_refCX_shr.tsv")
...
# collapsedPathways_OPC <- collapsePathways(df_tables_GSEA_all %>%
#                                            filter(dataset %in% "list_df_comparisons_OPC_old_young"), pathways, list_ranks$list_df_comparisons_OPC_old_young)
# glimpse(collapsedPathways_OPC)
...
# save the table
df_tables_GSEA_all_non_redundant %>%
  write_tsv("../../out/table/GSEA_REACTOME_DE_pseudobulk_OPC_ctrl_refCX_shr_nonredundant.tsv")
...
ggsave("../../out/image/GSEA_REACTOME_DE_pseudobulk_OPC_ctrl_refCX_shr_nonredundant.pdf",width = 15,height = 10)
...
df_tables_GSEA_all %>%
  ...
  geom_hline(yintercept = -log10(0.05),linetype="dashed",col="gray")+
  geom_text_repel(size = 2,box.padding = 0.5,segment.alpha = 0.6,max.overlaps = 10)
ggsave("../../out/image/GSEA_REACTOME_DE_pseudobulk_OPC_ctrl_refCX_shr.pdf",width = 15,height = 10)
