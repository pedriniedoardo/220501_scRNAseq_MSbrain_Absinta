...
df_en_090_1 <- df_modules %>% 
  select(barcode,signature,disease,seurat_clusters,signature_score1) %>% 
  filter(signature %in% "senmayo") %>% 
...
df_en_090_final %>%
  write_tsv("../../out/table/ManualClean/modules_SENESCENCE/enumeration_senmayo_090_threshold_MSStatus.tsv")
