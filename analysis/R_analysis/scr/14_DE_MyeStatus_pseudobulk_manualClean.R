...
data.combined$pathology_class <- str_replace_all(meta_full$pathology_class,pattern = " ",replacement = "_")
...
table(data.combined@meta.data$official_id,data.combined@meta.data$seurat_clusters,data.combined@meta.data$pathology_class)
...
data.combined$pseudobulk <- paste0(data.combined@meta.data$official_id,".",
                                   ...
                                   data.combined@meta.data$pathology_class)
...
matrix_counts %>%
  saveRDS("../../out/object/ManualClean/pseudobulk_MyeStatus_SoupX_manualClean_harmony.rds")

# save the aggregated mete
meta_summary <- data.combined@meta.data %>%
  group_by(official_id,pathology_class,sex,age,disease,orig.ident.cca,pseudobulk) %>%
  summarise()
meta_summary %>%
  write_tsv("../../out/table/ManualClean/pseudobulk_Meta_MyeStatus_SoupX_manualClean_harmony.tsv")
...
meta_summary <- read_tsv("../../out/table/ManualClean/pseudobulk_Meta_MyeStatus_SoupX_manualClean_harmony.tsv")
...
# build the design
# x <- list_colData$clust_0
list_design <- lapply(list_colData,function(x){
  treat <- factor(x$pathology_class,levels = c("control_cortex", "myelinated_cortex","demyelinated_cortex"))
  gender <- factor(x$sex)
  # age <- x$age
  ...
  colnames(design) <- c("intercept","genderM","treatMyelinated_cortex","treatDemyelinated_cortex")
  design
})
...
pmap(list(list_dds,names(list_dds),list_design),function(x,name_dds,design_dds){
  ...
  contrast <- makeContrasts(ReMyevsNorm = treatMyelinated_cortex,
                            DeMyevsNorm = treatDemyelinated_cortex,
                            DeMyevsReMye = treatDemyelinated_cortex - treatMyelinated_cortex,
                            levels = design_dds)
  
  # Generate results object
  res_ReMye <- results(dds2, contrast=contrast[,"ReMyevsNorm"],alpha = 0.05)
  res_DeMye <- results(dds2, contrast=contrast[,"DeMyevsNorm"],alpha = 0.05)
  res_DeMyeVsReMye <- results(dds2, contrast=contrast[,"DeMyevsReMye"],alpha = 0.05)
  
  summary(res_ReMye)
  summary(res_DeMye)
  summary(res_DeMyeVsReMye)
  
  res_ReMye %>% data.frame() %>% rownames_to_column("gene") %>% arrange(pvalue) %>%
    write_tsv(paste0("../../out/table/ManualClean/responsePseudobulk_MyeStatus_",name_dds,"_ReMye_vs_Norm.tsv"))
  
  res_DeMye %>% data.frame() %>% rownames_to_column("gene") %>% arrange(pvalue) %>%
    write_tsv(paste0("../../out/table/ManualClean/responsePseudobulk_MyeStatus_",name_dds,"_DeMye_vs_Norm.tsv"))
  
  res_DeMyeVsReMye %>% data.frame() %>% rownames_to_column("gene") %>% arrange(pvalue) %>%
    write_tsv(paste0("../../out/table/ManualClean/responsePseudobulk_MyeStatus_",name_dds,"_DeMye_vs_ReMye.tsv"))
  
  # LFC shrinkage
  res_ReMye_shr <- lfcShrink(dds2, res = res_ReMye, type = "ashr")
  res_DeMye_shr <- lfcShrink(dds2, res = res_DeMye, type = "ashr")
  res_DeMyeVsReMye_shr <- lfcShrink(dds2, res = res_DeMyeVsReMye, type = "ashr")
  
  res_ReMye_shr %>% data.frame() %>% rownames_to_column("gene") %>% arrange(pvalue) %>%
    write_tsv(paste0("../../out/table/ManualClean/responsePseudobulkShr_MyeStatus_",name_dds,"_ReMye_vs_Norm.tsv"))
  
  res_DeMye_shr %>% data.frame() %>% rownames_to_column("gene") %>% arrange(pvalue) %>%
    write_tsv(paste0("../../out/table/ManualClean/responsePseudobulkShr_MyeStatus_",name_dds,"_DeMye_vs_Norm.tsv"))
  
  res_DeMyeVsReMye_shr %>% data.frame() %>% rownames_to_column("gene") %>% arrange(pvalue) %>%
    write_tsv(paste0("../../out/table/ManualClean/responsePseudobulkShr_MyeStatus_",name_dds,"_DeMye_vs_ReMye.tsv"))
...
ggsave("../../out/image/ManualClean/volcano_pseudobulk_MyeStatus.pdf",width = 20,height = 10)

# unscale the volcanoes
df_res %>%
  # mutate(comparison = str_extract(annotation,pattern = "NOLD_vs_control|MCI_vs_control|NOLD_vs_MCI")) %>%
  # filter(symbol %in% setdiff(GOI_SLC,GOI)) %>%
  ggplot(aes(x = log2FoldChange,y = -log(padj))) +
  ...
  geom_hline(yintercept = (-log(0.05)),col="gray",linetype="dashed")+
  geom_text_repel(data = test_significant,aes(x = log2FoldChange,y = -log(padj),label = gene)) +
  facet_wrap(treat~cell_id,scales = "free") +
  theme_bw()+theme(strip.background = element_blank())
ggsave("../../out/image/ManualClean/volcano_pseudobulk_MyeStatus_ScalesFree.pdf",width = 20,height = 10)
