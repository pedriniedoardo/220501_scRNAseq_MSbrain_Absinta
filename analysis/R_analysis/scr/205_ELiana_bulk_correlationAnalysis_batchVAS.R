...
vst_batch_VAS <- readRDS("../../out/object/204_vst_filterBatch_Eliana_Absinta_CellidSmall_VAS.rds")
# vst_batch_VAS <- readRDS("../../out/object/204_vst_filterBatch_Eliana_Absinta_CellidSmall.rds")

# processing --------------------------------------------------------------
# produce the matrix and metadata
vst_batch_VAS_mat <- assay(vst_batch_VAS)
metadata_VAS <- colData(vst_batch_VAS)
...
correlation_matrix_VAS <- cor(vst_batch_VAS_mat,method = "spearman")

# Visualize the results as a heatmap
pdf("../../out/image/205_heatmap_all_correlation_batch_VAS.pdf",width = 7,height = 6)
Heatmap(correlation_matrix_VAS,col = viridis::viridis(option = "turbo",n = 10))
dev.off()
...
bulk_samples <- rownames(metadata_VAS[metadata_VAS$batch == "Eliana", ])
sc_samples <- rownames(metadata_VAS[metadata_VAS$batch == "Absinta", ])

# Calculate Spearman correlation
correlation_matrix_VAS2 <- cor(vst_batch_VAS_mat[, bulk_samples], 
                               vst_batch_VAS_mat[, sc_samples], 
                               method = "spearman")

# Visualize the results as a heatmap
pdf("../../out/image/205_heatmap_short_correlation_batch_VAS.pdf",width = 7,height = 6)
