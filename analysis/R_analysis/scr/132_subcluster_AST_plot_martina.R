# AIM ---------------------------------------------------------------------
# the aim is to plot the data after integration of the subclusters of AST cells
...
#                    prefix = "RNA_snn_res.")
# ggsave("../../out/image/132_UMAPCluster_tree_AST_subcluster.pdf",width = 10,height = 10)

# general UMAP with new clustering
# DimPlot(data.combined, reduction = "umap",group.by = "RNA_snn_res.0.1",label = T,raster = T)
# ggsave("../../out/image/132_UMAPCluster_res0.1_AST_subcluster.pdf",width = 4,height = 3)

DimPlot(data.combined, reduction = "umap",group.by = "cellid",label = T,raster = T)
ggsave("../../out/image/132_UMAPCluster_cellid_AST_subcluster.pdf",width = 5,height = 4)

DimPlot(data.combined, reduction = "umap",group.by = "cellid",split.by = "origin",label = T,raster = T,ncol=4)
ggsave("../../out/image/132_UMAPCluster_splitTreat_cellid_AST_subcluster.pdf",width = 12,height = 6)

DimPlot(data.combined, reduction = "umap",group.by = "cellid",split.by = "origin",label = F,raster = T,ncol=2,pt.size = 2)
ggsave("../../out/image/132_UMAPCluster_splitTreat_cellid_AST_subcluster2.pdf",width = 6,height = 3)
...
# ggsave("../../out/image/132_UMAPCluster_resolutions_AST_subcluster.pdf",width = 25,height = 15)
...
ggsave(plot=test_long01,"../../out/image/132_DotplotLong_cellid_AST_subcluster.pdf",width = 30,height = 5)
...
# ggsave("../../out/image/132_UMAPCluster_short_AST_subcluster.pdf",width = 22,height = 12)
...
# ggsave("../../out/image/132_UMAPCluster_long_AST_subcluster.pdf",width = 22,height = 12)
...
df_signatureAbsinta <- data.combined@meta.data %>%
  dplyr::select(cellid,contains("scoreLong_")) %>%
...
pdf("../../out/image/132_heatmap_signatureLong_cellid_ASTsubset.pdf",width = 10,height = 4)
...
pdf("../../out/image/132_heatmap_signatureLong_cellid_ASTsubset2.pdf",width = 8,height = 5)
...
sobj_total_h.markers %>%
  write_tsv("../../out/table/132_FindAllMarkers_HarmonySample_cellid_AST_subcluster.tsv")

# pick the top 100 markers per cluster
sobj_total_h.markers %>%
  group_by(cluster) %>%
  dplyr::slice(1:100) %>%
  write_tsv("../../out/table/132_FindAllMarkers_HarmonySample_cellid_AST_subcluster_top100.tsv")
...
sobj_total_h.markers %>%
  ...
  write_tsv("../../out/table/132_FindAllMarkers_HarmonySample_cellid_AST_subcluster_top100_noRIBOandMT.tsv")
...
dittoSeq::dittoDotPlot(data.combined,
                       ...
                       group.by = "cellid")+scale_color_viridis_c(option = "turbo",name="relative \nexpression")
ggsave("../../out/image/132_Ditto_HarmonySample_cellid_AST_subcluster.pdf",width = 15,height = 5)

# plot the proportions
df_summary <- data.combined@meta.data %>%
  group_by(dataset,pathology_class,origin,cellid) %>%
  summarise(n = n()) %>%
  ungroup() %>%
...
df_summary %>%
  ...
  facet_wrap(~cellid,ncol = 1,scales = "free")+
  scale_y_sqrt()
# ggsave("../../out/image/132_plot_clusterProp_res0.1_AST_subcluster.pdf",height = 20,width = 4)
...
out_diagnosis <- propeller(clusters = meta_test$cellid,
                           sample = paste0(meta_test$dataset),
                           group = meta_test$pathology_class)

# out_diagnosis %>%
#   rownames_to_column("RNA_snn_res.0.1") %>%
#   write_tsv("../../out/table/132_propeller_res0.1_AST_subcluster.tsv")

# plotting diagnosis ------------------------------------------------------
# default plot
speckle::plotCellTypeProps(x = data.combined,
                           clusters = data.combined$cellid,
...
# ggsave("../../out/image/132_plot_propeller_res0.1_AST_subcluster.pdf",height = 5,width = 5)
...
#   write_tsv("../../out/table/132_df_summary_diagnosis_res0.1_AST_subcluster.tsv")
...
df_summary_diagnosis %>%
  ...
  theme_bw()+
  theme(strip.background = element_blank(),axis.text.x = element_text(hjust = 1,angle = 45))
# ggsave("../../out/image/132_propeller_plot01_AST_subcluster.pdf",width = 10,height = 10)

# plot 02
df_summary_diagnosis %>%
  ggplot() +
  geom_boxplot(aes(x=cellid,y=prop,color=pathology_class),outlier.shape = NA) +
  geom_point(aes(x=cellid,y=prop,color=pathology_class),position = position_jitterdodge(jitter.width = 0.1,dodge.width = 0.8),alpha=0.7) +
  theme_cowplot()+
