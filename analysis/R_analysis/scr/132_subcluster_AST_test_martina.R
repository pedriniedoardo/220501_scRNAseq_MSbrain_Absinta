...
sobj_AST <- readRDS("../../out/object/132_AST_subcluster_HarmonySample_martina.rds")
DimPlot(sobj_AST,group.by = "cellid",raster = T)

# wrangling ---------------------------------------------------------------
# add the rename cluster Id from the res 0.1 to the original object
meta_test_subcluster <- sobj_AST@meta.data %>%
  rownames_to_column("barcode") %>%
  dplyr::select(clu_AST_sub = cellid,barcode)

# add the annotation from the cubsluster to the original object
# notice that some cells have been filtered out, but are still in the total object. remove them
sobj_tot@meta.data$test_cluster <- sobj_tot@meta.data %>%
  ...
  mutate(test_cluster = case_when(is.na(clu_AST_sub) ~ expertAnno.l1,
                                  ...
                                  T~clu_AST_sub)) %>%
  # remove the one that have the AST annotation (the left over of the filterd out cells)
  mutate(test_cluster = case_when(test_cluster=="AST" ~ "AST_removed",
                                  # is.na(clu_MG_sub) & expertAnno.l1 != "MG"~expertAnno.l1,
                                  # is.na(clu_MG_sub) & expertAnno.l1 == "MG"~"MG_cleaned",
                                  T~test_cluster)) %>%
  pull(test_cluster)
...
test_long_run032_alt <- df_test_run032 %>%
  # force the order
  mutate(id = factor(id,levels = c("Ciliated","Homeo","Stressed","Cortical AST","AIMS","Reactive","Perinodal","AST_removed","IMM","LYM","INH NEU","EXC NEU","VAS","OLIGO","OPC","EPENDYMA"))) %>%
  mutate(cell_type = factor(cell_type,levels = c("IMMUNE","B_CELLS","T_CELLS","NEURONS","ASTRO","OLIGOLINEAGE","ENDO","PERICYTE","SCHWANN","EPENDYMA","STROMAL"))) %>% 
  ggplot(aes(x = features.plot,y = id)) +
  geom_point(aes(size = pct.exp, col = avg.exp.scaled))+
  scale_radius(range = c(0, 8)) +
...
ggsave(plot = (test_long_run03_alt / test_long_subMG_alt / test_long_run032_alt),"../../out/image/132_DotplotLong_AST_subcluster_test_highres.pdf",width = 20,height = 20)

# plot also a specific panel for the dotplot ------------------------------
# this panel comes from Martina
shortlist_martina <- list(
  Cortical_AST = c("CABLES1", "GPC5", "BRINP3"),
  Stressed  = c("MT-CO3", "MT-ATP6"),
  Reactive = c("NAMPT", "CD44", "CRYAB"),
  Homeo = c("SLC24A4", "SLC38A1", "VCAN", "ADAMTSL3"),
  AIMS = c("VIM", "FTL", "APOE", "SPARC","GFAP"),
  Ciliated = c("CFAP54", "CFAP44", "DNAH11"),
  Perinodal = c("IL1RAPL1", "CTNNA3", "ST18")
)


test_long_subAST_martina <- DotPlot(sobj_AST,
                                    features = shortlist_martina,
                                    dot.scale = 8,
                                    cluster.idents = T,
                                    group.by = "cellid") +
  RotatedAxis() +
...
# generate the ggplot object
# force the order suggested by matrina
df_test_subAST_martina <- lapply(shortlist_martina,function(x){
  test_long_subAST_martina$data %>% 
    filter(features.plot %in% x)
}) %>% 
  bind_rows(.id = "cell_type")

# plot mapping radius
test_long_subMG_alt_martina <- df_test_subAST_martina %>%
  # force the order
  # mutate(id = factor(id,levels = c("IMM","LYM","INH NEU","EXC NEU","AST","OLIGO","OPC","AST","EPENDYMA"))) %>% 
  mutate(cell_type = factor(cell_type,levels = c("Reactive",
                                                 "AIMS",
                                                 "Ciliated",
                                                 "Perinodal",
                                                 "Homeo",
  ...
  theme(strip.background = element_blank(),axis.text.x = element_text(hjust = 1,angle = 90),strip.text = element_text(hjust = 0,angle = 90))+
  scale_color_gradient(low = "lightgrey",high = "blue")
ggsave(plot = test_long_subMG_alt_martina,"../../out/image/132_DotplotLong_AST_subcluster_test_martina.pdf",width = 8,height = 4)
