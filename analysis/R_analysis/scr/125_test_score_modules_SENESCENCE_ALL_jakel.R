...
# read the data -----------------------------------------------------------
# read in the dataset
data.combined <- readRDS("../../data/jakel_clean.rds")
# make sure the RNA assay is loaded
DefaultAssay(data.combined) <- "RNA"

# add more annotations
# add the macro annotation from the cellid, based on martina's paper
data.combined$cellid <- data.combined@meta.data %>%
  mutate(cellid = case_when(seurat_clusters %in% c(0,13)~"OLIGO",
                            seurat_clusters %in% c(5,6)~"VAS",
                            # seurat_clusters %in% c(19)~"LYMPHO",
                            seurat_clusters %in% c(4,9)~"IMM",
                            seurat_clusters %in% c(2)~"AST",
                            seurat_clusters %in% c(11,12)~"OPC",
                            seurat_clusters %in% c(7)~"INH NEU",
                            seurat_clusters %in% c(1,3,8,10)~"EXC NEU")) %>%
  pull(cellid)

# fix the order of the variable in the metadata
data.combined$disease <- data.combined@meta.data %>%
  mutate(disease = case_when(pathology %in% c("a_control")~"CTRL",
                             T~"MS")) %>%
  pull(disease)

#
data.combined$pathology <- data.combined@meta.data %>%
  mutate(pathology = factor(pathology,
                            levels = c("a_control","b_NAWM","d_chronic_inactive","A","c_chronic_active","RM"),
                            labels = c("CTRL","NAWM","CI","A","CA","RM"))) %>%
  pull(pathology)

# DimPlot(data.combined,reduction = "curated",label = T)
...
lapply(names(list_sig),function(x){
  ...
  df_meta %>% 
    write_tsv(paste0("../../out/table/revision/modules_SENESCENCE_jakel/120_Module_score_",x,".tsv"))
  ...
  data2 %>%
    ...
    # scale_color_gradientn(colours = c("blue","gray","red"))
    scale_color_gradientn(colours = viridis::turbo(10))
  ggsave(paste0("../../out/image/revision/modules_SENESCENCE_jakel/120_UMAP_score_",x,".pdf"),width = 12,height = 3)
  ggsave(paste0("../../out/image/revision/modules_SENESCENCE_jakel/120_UMAP_score_",x,".png"),width = 12,height = 3)
  ...
  data2 %>%
    ...
    # scale_color_gradientn(colours = c("blue","gray","red"))
    scale_color_gradientn(colours = viridis::turbo(10))
  ggsave(paste0("../../out/image/revision/modules_SENESCENCE_jakel/120_UMAP_score_",x,"_2.pdf"),width = 7,height = 3)
  ggsave(paste0("../../out/image/revision/modules_SENESCENCE_jakel/120_UMAP_score_",x,"_2.png"),width = 7,height = 3)
  ...
  data2 %>%
    ...
    # scale_color_gradientn(colours = c("blue","gray","red"))
    scale_color_gradientn(colours = viridis::turbo(10))
  ggsave(paste0("../../out/image/revision/modules_SENESCENCE_jakel/120_UMAP_score_split_",x,".pdf"),width = 20,height = 20)
  ggsave(paste0("../../out/image/revision/modules_SENESCENCE_jakel/120_UMAP_score_split_",x,".png"),width = 20,height = 20)
  ...
  # ggsave(paste0("../../out/image/ManualClean/modules_SENESCENCE_jakel/HVG4000/dist_score_",x,"_2.pdf"),width = 12,height =9)
  ...
  ggsave(paste0("../../out/image/revision/modules_SENESCENCE_jakel/120_dist_score_ridges_",x,"_2.pdf"),width = 12,height =9)
  ...
  ggsave(paste0("../../out/image/revision/modules_SENESCENCE_jakel/120_dist_score_ridges_",x,".pdf"),width = 12,height =9)
  ...
  # ggsave(paste0("../../out/image/ManualClean/modules_SENESCENCE_jakel/HVG4000/violin_score_split_",x,".pdf"),width = 25,height = 9)
  ...
  data2 %>%
    ...
    # scale_fill_manual(values = c("green","yellow","red"))+
    theme(strip.background = element_blank(),axis.text.x = element_text(hjust = 1,angle = 90))
  ggsave(paste0("../../out/image/revision/modules_SENESCENCE_jakel/120_violin_score_",x,".pdf"),width = 9,height = 9)
})
...
# scale_color_gradientn(colours = viridis::turbo(10))
ggsave(paste0("../../out/image/revision/modules_SENESCENCE_jakel/119_UMAP_score_",x,"_tailored1.pdf"),width = 5,height = 6)
ggsave(paste0("../../out/image/revision/modules_SENESCENCE_jakel/119_UMAP_score_",x,"_tailored1.png"),width = 5,height = 6,bg="white")
...
ggsave(paste0("../../out/image/revision/modules_SENESCENCE_jakel/119_UMAP_score_",x,"_tailored2.pdf"),width = 5,height = 6)
ggsave(paste0("../../out/image/revision/modules_SENESCENCE_jakel/119_UMAP_score_",x,"_tailored2.png"),width = 5,height = 6,bg="white")
...
# library(UpSetR)
# pdf("../../out/image/revision/modules_SENESCENCE_jakel/120_upset_signatures_shortlist.pdf",width = 5,height = 4,onefile = F)
...
# df_int %>%
#   write_tsv("../../out/table/revision/modules_SENESCENCE_jakel/120_upset_signatures_shortlist_intersection.tsv")
