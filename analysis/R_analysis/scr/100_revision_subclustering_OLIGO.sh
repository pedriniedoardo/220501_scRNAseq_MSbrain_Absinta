#!/bin/bash
#SBATCH --job-name=subcluster
#SBATCH --account pedrini.edoardo
#SBATCH --mem=64GB  # amout of RAM in MB required (and max ram available).
#SBATCH --time=INFINITE  ## OR #SBATCH --time=10:00 means 10 minutes OR --time=01:00:00 means 1 hour
#SBATCH --ntasks=8  # number of required cores
#SBATCH --nodes=1  # not really useful for not mpi jobs
#SBATCH --mail-type=FAIL ## BEGIN, END, FAIL or ALL
#SBATCH --mail-user=pedrini.edoardo@hsr.it
#SBATCH --error="100_revision_subclustering_OLIGO.err"
#SBATCH --output="100_revision_subclustering_OLIGO.out"

echo "my job strart now" > 100_revision_subclustering_OLIGO.log;

date >> 100_revision_subclustering_OLIGO.log;

. /home/pedrini.edoardo/miniconda3/bin/activate;
conda activate env_R4.2_recover2;

Rscript /beegfs/scratch/ric.cosr/pedrini.edoardo/project_edoardo/220501_scRNAseq_MSbrain_Absinta/analysis/R_analysis/scr/100_revision_subclustering_OLIGO.R

date >> 100_revision_subclustering_OLIGO.log;
echo "all done!!" >> 100_revision_subclustering_OLIGO.log