...
(DimPlot(data.combined,group.by = "cellid") + ggtitle("Harmony Sample"))
(DimPlot(data.combined,group.by = "cellid") + ggtitle("Harmony Sample"))
...
df_meta_full %>%
  ...
  ungroup() %>%
  mutate(cellid = fct_reorder(cellid,-avg_prop)) %>%
  ggplot(aes(x=cellid,y=prop,fill=var))+geom_col(position = "dodge")+theme_cowplot()
...
df_meta_full %>%
  ...
  ungroup() %>%
  mutate(cellid = fct_reorder(cellid,-avg_prop)) %>%
  write_tsv("../../out/table/132_prop_cellid_senescence_AST.tsv")

# add the sample info
df_meta_full %>%
  group_by(orig.ident,pathology_class,cellid) %>%
  summarise(n = n(),
            tot_sen_SIT = sum(sen_SIT),
            tot_sen_SENMAYO = sum(sen_SENMAYO)) %>%
  ungroup() %>%
...
df_meta_full %>%
  ...
  ungroup() %>%
  mutate(cellid = fct_reorder(cellid,-avg_prop)) %>%
  ggplot(aes(x=cellid,y=prop,col=var)) +
  geom_boxplot(position = "dodge",outlier.shape = NA) +
...
df_meta_full %>%
  ...
  ungroup() %>%
  mutate(cellid = fct_reorder(cellid,-avg_prop)) %>%
  write_tsv("../../out/table/132_prop_cellid_senescence_sample_AST.tsv")
...
df_meta_full %>%
  ...
  mutate(cellid = fct_reorder(cellid,prop)) %>%
...
df_meta_full %>%
  ...
  mutate(prop = n_cluster/n_tot_sample) %>%
  mutate(cellid = fct_reorder(cellid,prop)) %>%
  write_tsv("../../out/table/132_prop_cellid_AST.tsv")

# split by origin and disease per cluster
df_meta_full %>%
  group_by(orig.ident,origin,disease,cellid) %>%
  summarise(n_cluster = n()) %>%
  ungroup() %>%
  group_by(orig.ident) %>%
  mutate(n_tot_sample = sum(n_cluster)) %>%
...
df_meta_full %>%
  ...
  mutate(prop = n_cluster/n_tot_sample) %>%
  mutate(cellid = fct_reorder(cellid,prop)) %>%
  write_tsv("../../out/table/132_prop_cellid_AST_originDisease.tsv")
...
df_meta_full %>%
  ...
  complete(orig.ident,cellid, fill = list(n_cluster = 0)) %>%
...
df_meta_full %>%
  ...
  mutate(prop = n_cluster/n_tot_sample) %>%
  mutate(cellid = fct_reorder(cellid,prop)) %>%
  write_tsv("../../out/table/132_prop_cellid_AST_includeZeros.tsv")
...
df_meta_full %>%
  ...
  mutate(prop = n_cluster/n_tot_sample) %>%
  mutate(cellid = fct_reorder(cellid,prop)) %>%
  write_tsv("../../out/table/132_prop_cellid_AST_originDisease_includeZeros.tsv")
