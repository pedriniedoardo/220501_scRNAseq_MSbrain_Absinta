# AIM ---------------------------------------------------------------------
# this script will split the object that martina as requested. in particular we are focussing on the CA samples only. In one object put all the cells from CA and the IMM cells called as senescent by SIT, in the other object the same cells, but the 
...
plot <- ComplexUpset::upset(fromList(list_barcodes),colnames(fromList(list_barcodes)),wrap=T) + ggtitle("IMM senescence")
ggsave(plot = plot,"../../out/image/revision/126_upset_senescence_signature_IMM.pdf",width = 5,height = 3)
