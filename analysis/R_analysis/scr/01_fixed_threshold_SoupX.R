...
colnames(list_datasc_fixed_norm_doublet$`11_69_control_cortex_martina_ref_CR6_1`@meta.data)
#
lapply(list_datasc_fixed_norm_doublet, function(x){
  meta <- x@meta.data
  colnames(meta) <- c("orig.ident","nCount_RNA","nFeature_RNA","percent.mt","percent.ribo","mt_bin","official_id","facility","origin","patient","pathology","disease","sex","age","PMI","year_of_extraction","test","not_outlier","RNA_snn_res.0.8","seurat_clusters","pANN","DF")
  x@meta.data <- meta
  head(x@meta.data)
  dim(x@meta.data)[1]
})
