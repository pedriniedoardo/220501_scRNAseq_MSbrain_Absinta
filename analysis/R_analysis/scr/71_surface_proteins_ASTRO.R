...
df <- read_tsv("../../out/table/DE_pseudobulk_ASTRO_ctrl_refCX_shr.tsv")
...
df_tot %>% 
  write_tsv("../../out/table/surfaceProt_pseudobulk_ASTRO_ctrl_refCX_shr.tsv")

# plot the expression data to focus only on the surface protein genes

# heatmaps ----------------------------------------------------------------
# save the filtered object
ddsHTSeq_ASTRO_filter <- readRDS("../../out/object/ddsHTSeq_pseudobulk_ASTRO_ctrl_refCX.rds")
colData_MG <- readRDS(file = "../../out/object/colData_ASTRO.rds")

# scale the data
vds_ASTRO_filter <- vst(ddsHTSeq_ASTRO_filter, blind = F)

# plot_vsd_MG <- plotPCA(vds_ASTRO_filter,
...
mat_filter_MG <- assay(vds_ASTRO_filter) %>%
  data.frame() %>%
...
mat_shr_MG <- mat_filter_MG[rownames(vds_ASTRO_filter) %in% DEG_2, ]
...
pdf("../../out/image/surfaceProt_heatmap_DEG_plot_pseudobulk_ASTRO_ctrl_refCX.pdf",width = 7,height = 25)
...
ggsave("../../out/image/surfaceProt_vulcano_plot_pseudobulk_ASTRO_ctrl_refCX.pdf",width = 10,height = 10)
