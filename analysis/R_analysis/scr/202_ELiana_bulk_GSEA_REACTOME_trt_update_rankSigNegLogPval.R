...
# this is for the trt vs ctrl comparison
...
file <- dir("../../out/table/") %>%
  str_subset(pattern = "res_TRT_shr") %>%
  str_subset(pattern = ".txt") %>%
...
df_tables_GSEA_all %>%
  write_tsv(paste0("../../out/table/202_df_table_GSEA_REACTOME_rankSigLogPval_","TRT",".tsv"))
...
# save the table
df_tables_GSEA_all_non_redundant %>%
  write_tsv(file = paste0("../../out/table/202_df_table_GSEA_REACTOME_nonredundant_rankSigLogPval_","TRT",".tsv"))
...
ggsave(paste0("../../out/image/202_GSEA_unbiased_REACTOME_nonredundant_rankSigLogPval_","TRT",".pdf"),width = 15,height = 15)
...
df_tables_GSEA_all %>%
  ...
  geom_text_repel(size = 2,box.padding = 0.5,segment.alpha = 0.6,max.overlaps = 10)+
  geom_hline(yintercept = -log(0.05),col="gray",linetype="dashed")
ggsave(paste0("../../out/image/202_GSEA_unbiased_REACTOME_rankSigLogPval_","TRT",".pdf"),width = 15,height = 15)
...
#
df_tables_GSEA_rankPvalue <- read_tsv(paste0("../../out/table/202_df_table_GSEA_REACTOME_rankSigLogPval_TRT.tsv")) %>%
  dplyr::select(dataset,pathway,NES,padj,leadingEdge)

df_tables_GSEA_rankLogFC <- read_tsv(paste0("../../out/table/202_df_table_GSEA_REACTOME_ranklogfc_TRT.tsv")) %>%
  dplyr::select(dataset,pathway,NES,padj,leadingEdge)
...
list_plot <- lapply(c("res_TRT_shr"),function(dat){
...
ggsave("../../out/image/202_GSEA_top20_REACTOME_TRT.pdf",width = 12,height = 12)

# non rudundant -----------------------------------------------------------
#
df_tables_GSEA_rankPvalue2 <- read_tsv(paste0("../../out/table/202_df_table_GSEA_REACTOME_nonredundant_rankSigLogPval_TRT.tsv")) %>%
  dplyr::select(dataset,pathway,NES,padj,leadingEdge)

df_tables_GSEA_rankLogFC2 <- read_tsv(paste0("../../out/table/202_df_table_GSEA_REACTOME_nonredundant_ranklogfc_TRT.tsv")) %>%
  dplyr::select(dataset,pathway,NES,padj,leadingEdge)
...
# dat <- "res_MS_shr"
list_plot2 <- lapply(c("res_TRT_shr"),function(dat){
...
ggsave("../../out/image/202_GSEA_top20_REACTOME_nonredundant_TRT.pdf",width = 12,height = 12)

# -------------------------------------------------------------------------
# check the IL13 signature
df_tables_GSEA_rankPvalue %>%
  filter(str_detect(pathway,pattern = "TP53"))
...
df_tables_GSEA_rankLogFC %>%
  filter(pathway == "REACTOME_REGULATION_OF_TP53_ACTIVITY_THROUGH_PHOSPHORYLATION")

df_tables_GSEA_rankLogFC2 %>%
  filter(str_detect(pathway,pattern = "TP53"))
