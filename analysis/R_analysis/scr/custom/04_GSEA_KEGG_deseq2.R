...
file <- "02_DE_pseudobulk_filterExp_shr.tsv"
...
df_tables_GSEA_all %>%
  write_tsv("../out/table/04_df_table_GSEA_deseq2_KEGG.tsv")
...
# save the table
df_tables_GSEA_all_non_redundant %>%
  write_tsv(file = "../out/table/04_df_table_GSEA_deseq2_KEGG_nonredundant.tsv")
...
ggsave("../out/plot/04_GSEA_unbiased_deseq2_KEGG_nonredundant.pdf",width = 15,height = 10)
...
df_tables_GSEA_all %>%
  ...
  geom_text_repel(size = 2,box.padding = 0.5,segment.alpha = 0.6,max.overlaps = 10)+
  geom_hline(yintercept = -log(0.05),col="gray",linetype="dashed")
ggsave("../out/plot/04_GSEA_unbiased_deseq2_KEGG.pdf",width = 15,height = 9)
