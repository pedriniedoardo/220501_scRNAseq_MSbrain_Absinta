# AIM ---------------------------------------------------------------------
# try the subclustering of the ATRO cells
...
# subset the cells
data.combined_subset <- subset(data.combined,subset = expertAnno.l1 == "ASTRO")
...
saveRDS(sobj_total,"../../out/object/ManualClean/data.combined_WM_CX_ASTRO_SkipIntegration.rds")
...
saveRDS(sobj_total_h,"../../out/object/ManualClean/data.combined_WM_CX_ASTRO_harmonySkipIntegration.rds")

# run DE per cluster ------------------------------------------------------
sobj_total_h.markers <- RunPrestoAll(sobj_total_h, only.pos = TRUE, min.pct = 0.25, logfc.threshold = 0.25)
write_tsv(sobj_total_h.markers,"../../out/table/ManualClean/FindAllMarkers_data.combined_WM_CX_ASTRO_harmonySkipInteg_PosOnly.tsv")
...
list_UP %>%
  write_tsv("../../out/table/ManualClean/enrichR_FindAllMarkers_WM_CX_ASTRO_harmonySkipInteg_PosOnly.tsv")

# explore a specific term
list_UP %>%
  filter(annotation %in% c("HDSigDB_Human_2021"),
         comparison %in% c(1))
# filter(str_detect(Term,pattern = "astro")) %>% 
...
ggsave("../../out/image/ManualClean/enrichR_FindAllMarkers_WM_CX_ASTRO_harmonySkipInteg_PosOnly.pdf",width = 20,height = 35,limitsize = FALSE)
