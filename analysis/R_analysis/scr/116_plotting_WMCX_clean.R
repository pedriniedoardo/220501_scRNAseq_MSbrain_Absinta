...
# read in the data --------------------------------------------------------
data.combined <- readRDS("../../out/object/revision/115_WMCX_ManualClean3_harmonySkipIntegration_AllSoupX_4000.rds")
...
clustree::clustree(data.combined@meta.data[,grep("RNA_snn_res", colnames(data.combined@meta.data))],
                   prefix = "RNA_snn_res.")
ggsave("../../out/image/revision/112_UMAPCluster_tree.pdf",width = 10,height = 10)
...
ggsave("../../out/image/revision/112_UMAPCluster_resolutions.pdf",width = 25,height = 15)
...
ggsave(plot=test_long01,"../../out/plot/manualClean/DotplotLong_harmonySkipIntegration_AllSoupX_00500_07000_05_res0.2.pdf",width = 30,height = 6)

test_long012 <- DotPlot(data.combined,
  ...
  labs(title = "RNA_snn_res.0.2")+
  theme(strip.text = element_text(angle = 90))
ggsave(plot=test_long012,"../../out/plot/manualClean/DotplotLong_harmonySkipIntegration_AllSoupX_00500_07000_05_res0.1_noNPC.pdf",width = 30,height = 6)

test_long042 <- DotPlot(data.combined,
                        features = shortlist_features_list_long2,
                        dot.scale = 8,
                        cluster.idents = T,
                        group.by = "RNA_snn_res.0.2") +
  ...
  labs(title = "RNA_snn_res.0.2") +
  theme(strip.text = element_text(angle = 90))
ggsave(plot=test_long042,"../../out/plot/manualClean/DotplotLong_harmonySkipIntegration_AllSoupX_00500_07000_05_res0.2_noNPC.pdf",width = 30,height = 6)
...
test_short04 <- DotPlot(data.combined,
  ...
  labs(title = "RNA_snn_res.0.2") +
  theme(strip.text = element_text(angle = 90))
ggsave(plot=test_short04,"../../out/plot/manualClean/DotplotShort_harmonySkipIntegration_AllSoupX_00500_07000_05_res0.2_noNPC.pdf",width = 15,height = 6)
...
ggsave("../../out/plot/manualClean/UMAPCluster_technical_res0.2.pdf",width = 10,height = 8)
...
df_plot_violin %>%
  ...
  theme(strip.background = element_blank(),
        axis.text.x = element_text(hjust = 1,angle = 45))
ggsave("../../out/plot/manualClean/ViolinCluster_technical_res0.2.pdf",width = 7,height = 12)

# pick one resolution onward 0.2

# main umap
plot03 <- DimPlot(data.combined, reduction = "umap", group.by = "RNA_snn_res.0.2",label = T,raster = T)
ggsave(plot=plot03,"../../out/plot/manualClean/UMAPCluster_harmonySkipIntegration_AllSoupX_00500_07000_05_res0.2.pdf",width = 6,height = 5)

plot03a <- DimPlot(data.combined, reduction = "umap", group.by = "RNA_snn_res.0.2",label = T,raster = T,split.by = "original_sample_name",ncol = 5)
ggsave(plot=plot03a,"../../out/plot/manualClean/UMAPClusterSplit_harmonySkipIntegration_AllSoupX_00500_07000_05_res0.2.pdf",width = 13,height = 12)

# main umap cell cycle
plot04 <- DimPlot(data.combined, reduction = "umap", group.by = "Phase",raster = T,order = T)
ggsave(plot=plot04,"../../out/plot/manualClean/UMAPPhase_harmonySkipIntegration_AllSoupX_00500_07000_05.pdf",width = 5,height = 4)
...
df_summary_phase <- df_meta %>%
  group_by(RNA_snn_res.0.2,Phase) %>%
  summarise(n = n()) %>%
...
df_summary_phase %>%
  mutate(RNA_snn_res.0.2 = fct_relevel(RNA_snn_res.0.2,as.character(0:15))) %>%
  ggplot() +
  geom_col(aes(x=RNA_snn_res.0.2,y=prop,fill=Phase))+theme_cowplot()+
  theme(axis.text.x = element_text(hjust = 1,angle = 90))
ggsave("../../out/plot/manualClean/BarplotPhase_summary_harmonySkipIntegration_AllSoupX_00500_07000_05_res0.2.pdf",width = 7,height = 6)

# split by donor
df_summary_phase2 <- df_meta %>%
  group_by(original_sample_name,RNA_snn_res.0.2,Phase) %>%
  summarise(n = n()) %>%
  group_by(original_sample_name,RNA_snn_res.0.2) %>%
  mutate(tot=sum(n)) %>%
  ungroup() %>%
  mutate(prop=n/tot)
...
df_summary_phase2 %>%
  ...
  theme(axis.text.x = element_text(hjust = 1,angle = 90)) +
  facet_wrap(~RNA_snn_res.0.2,ncol = 1)+theme(strip.background = element_blank())
ggsave("../../out/plot/manualClean/BarplotPhase2_summary_harmonySkipIntegration_AllSoupX_00500_07000_05_res0.2.pdf",width = 7,height = 20)
...
df_summary <- df_meta %>%
  ...
  ungroup() %>%
  mutate(prop=n/tot)
write_tsv(df_summary,"../../out/table/manualClean/summary_harmonySkipIntegration_AllSoupX_00500_07000_05_res0.2.tsv")

color_id <- alphabet(length(unique(df_summary$RNA_snn_res.0.2)))
# check the colors
show_col(color_id)

df_summary %>%
  ggplot() +
  geom_col(aes(x=original_sample_name,y=prop,fill=RNA_snn_res.0.2))+theme_cowplot()+
  theme(axis.text.x = element_text(hjust = 1,angle = 90)) +
  scale_fill_manual(values = unname(color_id))
ggsave("../../out/plot/manualClean/Barplot_summary_harmonySkipIntegration_AllSoupX_00500_07000_05_res0.2.pdf",width = 7,height = 6)

# following Martina's reccomandation I added the info for diagnosis or location
df_summary_02 <- df_meta %>%
  group_by(original_sample_name,RNA_snn_res.0.2,location,diagnosis) %>%
  summarise(n = n()) %>%
  group_by(original_sample_name) %>%
  mutate(tot=sum(n)) %>%
  ungroup() %>%
  mutate(prop=n/tot)
write_tsv(df_summary_02,"../../out/table/manualClean/summary02_harmonySkipIntegration_AllSoupX_00500_07000_05_res0.2.tsv")

df_summary_02 %>%
  mutate(RNA_snn_res.0.2 = fct_relevel(RNA_snn_res.0.2,as.character(0:12))) %>%
  mutate(location = fct_relevel(location,c("cervical","thoracic","lumbar"))) %>%
  ggplot() +
  geom_boxplot(aes(x=RNA_snn_res.0.2,y=prop,color=location),outlier.shape = NA) +
  geom_point(aes(x=RNA_snn_res.0.2,y=prop,color=location),position = position_jitterdodge(jitter.width = 0.1,dodge.width = 0.8),alpha=0.7) +
  theme_cowplot()+
  theme(axis.text.x = element_text(hjust = 1,angle = 90))+
  scale_y_sqrt()
ggsave("../../out/plot/manualClean/BoxplotLocation_summary_harmonySkipIntegration_AllSoupX_00500_07000_05_res0.2.pdf",width = 8,height = 5)

df_summary_02 %>%
  mutate(RNA_snn_res.0.2 = fct_relevel(RNA_snn_res.0.2,as.character(0:12))) %>%
  ggplot() +
  geom_boxplot(aes(x=RNA_snn_res.0.2,y=prop,color=diagnosis),outlier.shape = NA) +
  geom_point(aes(x=RNA_snn_res.0.2,y=prop,color=diagnosis),position = position_jitterdodge(jitter.width = 0.1,dodge.width = 0.8),alpha=0.7) +
  theme_cowplot()+
  theme(axis.text.x = element_text(hjust = 1,angle = 90))+
  scale_y_sqrt()
ggsave("../../out/plot/manualClean/BoxplotDiagnosis_summary_harmonySkipIntegration_AllSoupX_00500_07000_05_res0.2.pdf",width = 8,height = 5)
...
sample_prop_wide <- df_summary_02 %>%
  ...
  # make it long
  dplyr::select(original_sample_name,RNA_snn_res.0.2,zscore) %>%
...
pdf("../../out/plot/manualClean/HeatmapCluster_summary_harmonySkipIntegration_AllSoupX_00500_07000_05_res0.2.pdf",width = 10,height = 6)
...
pdf("../../out/plot/manualClean/HeatmapSample_summary_harmonySkipIntegration_AllSoupX_00500_07000_05_res0.2.pdf",width = 10,height = 6)
...
sobj_total_h.markers %>%
  write_tsv("../../out/table/manualClean/FindAllMarkers_harmonySkipIntegration_AllSoupX_00500_07000_05_res0.2.tsv")

# save the top 100
sobj_total_h.markers %>%
  group_by(cluster) %>%
  dplyr::slice(1:100) %>%
  write_tsv("../../out/table/manualClean/FindAllMarkers_harmonySkipIntegration_AllSoupX_00500_07000_05_res0.2_top100.tsv")
...
ggsave("../../out/plot/manualClean/TopMarkersDitto_harmonySkipIntegration_AllSoupX_00500_07000_05_res0.2.pdf",width = 15,height = 6)
