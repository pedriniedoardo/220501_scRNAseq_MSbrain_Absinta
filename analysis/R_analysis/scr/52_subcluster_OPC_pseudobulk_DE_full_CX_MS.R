...
counts_OPC <- bind_cols(counts_OPC_Ctrl,counts_OPC_Mye) %>%
  select(contains("CX_"))
...
saveRDS(counts_OPC,file = "../../out/object/counts_OPC_full_CX.rds")
saveRDS(colData_OPC,file = "../../out/object/colData_OPC_full_CX.rds")
...
# save the disign
saveRDS(design_OPC,"../../out/object/design_OPC_full_CX.rds")
...
pdf("../../out/image/heatmap_SampleCluster_pseudobulk_OPC_full_CX.pdf",width = 10,height = 6)
set.seed(3)
...
# ggsave("../../out/image/PCA_pseudobulk_OPC_ctrl_refCX_test.pdf",width = 6,height = 4)
...
plot_vsd_OPC$data %>%
  ...
  scale_x_continuous(expand = expansion(mult = 0.1))+
  theme_bw() + ylab(plot_vsd_OPC$labels[1]) + xlab(plot_vsd_OPC$labels[2])
ggsave("../../out/image/PCA_pseudobulk_OPC_full_CX.pdf",width = 6,height = 4)
...
pdf("../../out/image/heatmap_cellType_pseudobulk_OPC_full_CX.pdf",width = 7,height = 5)
...
ht2_shr_OPC22 <- Heatmap(sample_prop_wide2,
                           show_column_names = T,
                           raster_by_magick = T,
...
pdf("../../out/image/heatmap_cellType_pseudobulk_OPC_full_CX2.pdf",width = 7,height = 5)
draw(ht2_shr_OPC22,heatmap_legend_side = "left",annotation_legend_side = "left",padding = unit(c(30,2,2, 2), "mm"))
dev.off()
...
saveRDS(ddsHTSeq_OPC_filter,"../../out/object/ddsHTSeq_pseudobulk_OPC_full_CX.rds")
...
contrast_OPC <- makeContrasts(OPC_MS_vs_CTRL = treat_fullMS,
...
# save the table
df_res %>%
  write_tsv("../../out/table/DE_pseudobulk_OPC_full_CX.tsv")
...
df_res_shr <-
  list(OPC.CX.MS_shr = res_OPC.MS_shr) %>%
  lapply(function(x){
    df <- x %>%
      data.frame() %>%
      rownames_to_column(var = "symbol") %>%
...
# save the table
df_res_shr %>%
  write_tsv("../../out/table/DE_pseudobulk_OPC_full_CX_shr.tsv")
...
ggsave("../../out/image/histogram_pvalue_pseudobulk_OPC_full_CX.pdf",width = 6,height = 4)
...
plot_volcano %>%
  ...
  ggrepel::geom_text_repel(
    data = plot_volcano[plot_volcano$col==1,] %>% group_by(conditionVsCTRL) %>% arrange(padj) %>% dplyr::slice(1:10),
    aes(label = symbol),segment.alpha=0.4) +
  facet_wrap(~conditionVsCTRL)+
  theme_bw()+
  theme(strip.background = element_blank())+
  theme(legend.position = "none")
ggsave("../../out/image/vulcano_plot_pseudobulk_OPC_full_CX.pdf",width = 10,height = 10)
...
column_ha_shr_OPC <- HeatmapAnnotation(disease = meta_sample_OPC$disease,
                                         gender = meta_sample_OPC$sex,
                                         origin = meta_sample_OPC$origin_01,
...
ht2_shr_OPC <- Heatmap(mat2_shr_OPC, show_column_names = T,raster_by_magick = T,show_row_dend = F, use_raster = T,
                         ...
                         # row_names_gp = gpar(fontsize = 3),
                         top_annotation = column_ha_shr_OPC,show_row_names = T,
                         # cluster_rows = F,
                         # right_annotation = row_ha,
                         # row_split = rep(c(1,2,3,4),c(2,3,4,7))
                         
)
pdf("../../out/image/heatmap_DEG_plot_pseudobulk_OPC_full_CX_shr.pdf",width = 7,height = 7)
...
# make the column of the matrix more readable
colnames(mat2_shr_OPC2) <- meta_sample_OPC$colname

column_ha_shr_OPC2 <- HeatmapAnnotation(disease = meta_sample_OPC2$disease,
                                          gender = meta_sample_OPC2$sex,
                                          origin = meta_sample_OPC2$origin_01,
                                          col = list(origin = c("CX" = "yellow",
                                                                "WM" = "blue"),
                                                     disease = c("MS" = "red",
                                                                 "CTRL" = "green"),
...
ht2_shr_OPC2 <- Heatmap(mat2_shr_OPC2, show_column_names = T,raster_by_magick = T,show_row_dend = F, use_raster = T,
                          ...
                          column_title = "OPC",
                          # row_names_gp = gpar(fontsize = 3),
                          top_annotation = column_ha_shr_OPC2,show_row_names = T,
                          # cluster_rows = F,
                          # right_annotation = row_ha,
                          # row_split = rep(c(1,2,3,4),c(2,3,4,7))
                          
)
pdf("../../out/image/heatmap_DEG_plot_pseudobulk_OPC_full_CX.pdf",width = 7,height = 10)
draw(ht2_shr_OPC2,heatmap_legend_side = "left",annotation_legend_side = "left",padding = unit(c(30,2,2, 2), "mm"))
dev.off()
...
# # PLOT DISPERSION ---------------------------------------------------------
# pdf("../../out/image/dispersion_plot_pseudobulk_OPC_ctrl_refCX_test.pdf",width = 5,height = 5)
