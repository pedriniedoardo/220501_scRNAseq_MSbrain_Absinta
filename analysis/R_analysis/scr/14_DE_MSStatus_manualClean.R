...
table(data.combined@meta.data$seurat_clusters,data.combined@meta.data$disease)

data.combined$condition.annotation <- paste(data.combined$disease,data.combined$seurat_clusters, sep = "_")
head(data.combined@meta.data)
...
list_MS_vs_CTRL %>%
  ...
  write_tsv("../../out/table/ManualClean/response_MS_vs_CTRL_manualClean_harmony.tsv")
...
ggsave("../../out/image/ManualClean/dist_p_value_MS_vs_CTRL_manualClean_harmony.pdf",width = 20,height = 4)
