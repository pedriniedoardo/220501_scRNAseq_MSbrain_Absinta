...
df_senescence <- data.combined@meta.data %>%
  dplyr::select(cellid,contains("scoreSen_")) %>%
...
pdf("../../out/image/132_heatmap_senescence_cellid_ASTsubset.pdf",width = 10,height = 5)
...
df_senescence %>%
  ...
  mutate(scaled_avg_score = (avg_score - mean(avg_score))/sd(avg_score)) %>%
  ungroup() %>%
  write_tsv("../../out/table/132_heatmap_table_senescence_cellid_ASTsubset.tsv")
