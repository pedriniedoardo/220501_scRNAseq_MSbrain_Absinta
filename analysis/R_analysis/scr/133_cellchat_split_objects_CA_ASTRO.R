# AIM ---------------------------------------------------------------------
# this script will split the object that martina as requested. in particular we are focussing on the CA samples only. In one object put all the cells from CA and the AST cells called as senescent, in the other object the same cells, but the AST not called as senescent
...
list_barcodes <- list(senmayo = LUT_senmayo %>%
                        filter(pathology_class == "WM_CA",
                               expertAnno.l1 == 'AST',sen == 1) %>%
                        pull(barcode),
                      SIT = LUT_SIT %>%
                        filter(pathology_class == "WM_CA",
                               expertAnno.l1 == 'AST',SENEQUANTILE == "YES") %>%
                        pull(rowname))
...
# upset(fromList(list_barcodes), order.by = "freq")
# dev.off()
plot <- ComplexUpset::upset(fromList(list_barcodes),colnames(fromList(list_barcodes)),wrap=T) + ggtitle("AST senescence")
ggsave(plot = plot,"../../out/image/revision/133_upset_senescence_signature_AST.pdf",width = 5,height = 3)
...
# 182+1010+2864+92+217+6541+570+791+
# 1981
# 1038
...
saveRDS(sobj_cellchat_senmayoCtrl,file = "../../out/object/revision/133_sobj_cellchat_senmayoCtrl_AST.rds")
saveRDS(sobj_cellchat_senmayoSen,file = "../../out/object/revision/133_sobj_cellchat_senmayoSen_AST.rds")
...
barcodes_senSIT <- LUT_SIT %>%
  ...
  mutate(keep = case_when(
    expertAnno.l1 != "AST" ~ 1,
    expertAnno.l1 == "AST" & SENEQUANTILE == "YES" ~ 1,
    T~0)) %>%
  filter(keep ==1) %>%
  pull(rowname)
...
barcodes_ctrSIT <- LUT_SIT %>%
  ...
  mutate(keep = case_when(
    expertAnno.l1 != "AST" ~ 1,
    expertAnno.l1 == "AST" & SENEQUANTILE == "NO" ~ 1,
    T ~ 0)) %>%
  filter(keep == 1) %>%
  pull(rowname)
...
saveRDS(sobj_cellchat_SITCtrl,file = "../../out/object/revision/133_sobj_cellchat_SITCtrl_AST.rds")
saveRDS(sobj_cellchat_SITSen,file = "../../out/object/revision/133_sobj_cellchat_SITSen_AST.rds")