...
ggsave("../../out/image/129_UMAPCluster_res0.1_MG_subcluster.pdf",width = 4,height = 3)

DimPlot(data.combined, reduction = "umap",group.by = "RNA_snn_res.0.1",split.by = "origin",label = T,raster = T,ncol=4)
ggsave("../../out/image/129_UMAPCluster_splitTreat_res0.1_MG_subcluster.pdf",width = 8,height = 4)
...
ggsave("../../out/image/129_UMAPCluster_resolutions_MG_subcluster.pdf",width = 25,height = 15)
...
ggsave("../../out/image/129_UMAPCluster_short_MG_subcluster.pdf",width = 22,height = 12)
...
sobj_total_h.markers %>%
  write_tsv("../../out/table/129_FindAllMarkers_HarmonySample_res0.1_MG_subcluster.tsv")
...
out_diagnosis %>%
  ...
  write_tsv("../../out/table/129_propeller_res0.1_MG_subcluster.tsv")
...
ggsave("../../out/image/129_plot_propeller_res0.1_MG_subcluster.pdf",height = 5,width = 5)
...
df_summary_diagnosis %>%
  write_tsv("../../out/table/129_df_summary_diagnosis_res0.1_MG_subcluster.tsv")
...
df_summary_diagnosis %>%
  ...
  theme_bw()+
  theme(strip.background = element_blank(),axis.text.x = element_text(hjust = 1,angle = 45))
ggsave("../../out/image/129_propeller_plot01_MG_subcluster.pdf",width = 10,height = 10)
