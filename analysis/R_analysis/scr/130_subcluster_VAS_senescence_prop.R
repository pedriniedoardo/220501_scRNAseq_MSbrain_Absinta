...
(DimPlot(data.combined,group.by = "RNA_snn_res.0.1") + ggtitle("Harmony Sample"))
...
df_meta_full %>%
  ...
  ungroup() %>%
  mutate(RNA_snn_res.0.1 = fct_reorder(RNA_snn_res.0.1,-avg_prop)) %>%
  ggplot(aes(x=RNA_snn_res.0.1,y=prop,fill=var))+geom_col(position = "dodge")+theme_cowplot()

# try at higher resolution
df_meta_full %>%
  ...
  mutate(avg_prop = mean(prop)) %>%
  ungroup() %>%
  mutate(RNA_snn_res.0.4 = fct_reorder(RNA_snn_res.0.4,-avg_prop)) %>%
  ggplot(aes(x=RNA_snn_res.0.4,y=prop,fill=var))+geom_col(position = "dodge")+theme_cowplot()
...
df_meta_full %>%
  ...
  ungroup() %>%
  mutate(RNA_snn_res.0.1 = fct_reorder(RNA_snn_res.0.1,-avg_prop)) %>%
  write_tsv("../../out/table/130_prop_res0.1_senescence_VAS.tsv")
...
df_meta_full %>%
  ...
  ungroup() %>%
  mutate(RNA_snn_res.0.4 = fct_reorder(RNA_snn_res.0.4,-avg_prop)) %>%
  write_tsv("../../out/table/130_prop_res0.4_senescence_VAS.tsv")

# add the sample info
df_meta_full %>%
  group_by(orig.ident,pathology_class,RNA_snn_res.0.1) %>%
  summarise(n = n(),
            tot_sen_SIT = sum(sen_SIT),
            tot_sen_SENMAYO = sum(sen_SENMAYO)) %>%
  ungroup() %>%
  ...
  ungroup() %>%
  mutate(RNA_snn_res.0.1 = fct_reorder(RNA_snn_res.0.1,-avg_prop)) %>%
  ggplot(aes(x=RNA_snn_res.0.1,y=prop)) +
...
df_meta_full %>%
  ...
  ungroup() %>%
  mutate(RNA_snn_res.0.1 = fct_reorder(RNA_snn_res.0.1,-avg_prop)) %>%
  ggplot(aes(x=RNA_snn_res.0.1,y=prop,col=var)) +
  geom_boxplot(position = "dodge",outlier.shape = NA) +
...
# higher resolution
df_meta_full %>%
  group_by(orig.ident,pathology_class,RNA_snn_res.0.4) %>%
  summarise(n = n(),
            tot_sen_SIT = sum(sen_SIT),
            tot_sen_SENMAYO = sum(sen_SENMAYO)) %>%
  ungroup() %>%
  ...
  ungroup() %>%
  mutate(RNA_snn_res.0.4 = fct_reorder(RNA_snn_res.0.4,-avg_prop)) %>%
  ggplot(aes(x=RNA_snn_res.0.4,y=prop,col=var)) +
  geom_boxplot(position = "dodge",outlier.shape = NA) +
  geom_point(position = position_jitterdodge(jitter.width = 0.2,dodge.width = 0.8),alpha=0.6) +
  theme_cowplot()+
  facet_wrap(~pathology_class,nrow=2)+
...
df_meta_full %>%
  ...
  ungroup() %>%
  mutate(RNA_snn_res.0.1 = fct_reorder(RNA_snn_res.0.1,-avg_prop)) %>%
  write_tsv("../../out/table/130_prop_res0.1_senescence_sample_VAS.tsv")
...
df_meta_full %>%
  ...
  ungroup() %>%
  mutate(RNA_snn_res.0.4 = fct_reorder(RNA_snn_res.0.4,-avg_prop)) %>%
  write_tsv("../../out/table/130_prop_res0.4_senescence_sample_VAS.tsv")
...
df_meta_full %>%
  ...
  mutate(RNA_snn_res.0.1 = fct_reorder(RNA_snn_res.0.1,prop)) %>%
...
df_meta_full %>%
  ...
  mutate(prop = n_cluster/n_tot_sample) %>%
  mutate(RNA_snn_res.0.1 = fct_reorder(RNA_snn_res.0.1,prop)) %>%
  write_tsv("../../out/table/130_prop_res0.1.tsv")
...
df_meta_full %>%
  ...
  mutate(prop = n_cluster/n_tot_sample) %>%
  mutate(RNA_snn_res.0.4 = fct_reorder(RNA_snn_res.0.4,prop)) %>%
  write_tsv("../../out/table/130_prop_res0.4.tsv")


# split by origin and disease per cluster
df_meta_full %>%
  group_by(orig.ident,origin,disease,RNA_snn_res.0.1) %>%
  summarise(n_cluster = n()) %>%
  ungroup() %>%
  group_by(orig.ident) %>%
  mutate(n_tot_sample = sum(n_cluster)) %>%
...
df_meta_full %>%
  group_by(orig.ident,origin,disease,RNA_snn_res.0.4) %>%
  summarise(n_cluster = n()) %>%
  ungroup() %>%
  group_by(orig.ident) %>%
  mutate(n_tot_sample = sum(n_cluster)) %>%
...
df_meta_full %>%
  ...
  mutate(prop = n_cluster/n_tot_sample) %>%
  mutate(RNA_snn_res.0.1 = fct_reorder(RNA_snn_res.0.1,prop)) %>%
  write_tsv("../../out/table/130_prop_res0.1_originDisease.tsv")
...
df_meta_full %>%
  ...
  mutate(prop = n_cluster/n_tot_sample) %>%
  mutate(RNA_snn_res.0.4 = fct_reorder(RNA_snn_res.0.4,prop)) %>%
  write_tsv("../../out/table/130_prop_res0.4_originDisease.tsv")
...
df_meta_full %>%
  ...
  complete(orig.ident,RNA_snn_res.0.1, fill = list(n_cluster = 0)) %>%
...
df_meta_full %>%
  group_by(orig.ident,pathology_class,RNA_snn_res.0.4) %>%
  summarise(n_cluster = n(),.groups = "drop") %>%
  complete(orig.ident,RNA_snn_res.0.4, fill = list(n_cluster = 0)) %>%
  select(-pathology_class) %>%
  left_join(LUT_sample,by = "orig.ident") %>%
  group_by(orig.ident) %>%
  mutate(n_tot_sample = sum(n_cluster)) %>%
...
df_meta_full %>%
  ...
  mutate(prop = n_cluster/n_tot_sample) %>%
  mutate(RNA_snn_res.0.1 = fct_reorder(RNA_snn_res.0.1,prop)) %>%
  write_tsv("../../out/table/130_prop_res0.1_includeZeros.tsv")
...
df_meta_full %>%
  ...
  mutate(prop = n_cluster/n_tot_sample) %>%
  mutate(RNA_snn_res.0.4 = fct_reorder(RNA_snn_res.0.4,prop)) %>%
  write_tsv("../../out/table/130_prop_res0.4_includeZeros.tsv")
...
df_meta_full %>%
  ...
  mutate(prop = n_cluster/n_tot_sample) %>%
  mutate(RNA_snn_res.0.1 = fct_reorder(RNA_snn_res.0.1,prop)) %>%
  write_tsv("../../out/table/130_prop_res0.1_originDisease_includeZeros.tsv")
...
df_meta_full %>%
  ...
  mutate(prop = n_cluster/n_tot_sample) %>%
  mutate(RNA_snn_res.0.4 = fct_reorder(RNA_snn_res.0.4,prop)) %>%
  write_tsv("../../out/table/130_prop_res0.4_originDisease_includeZeros.tsv")
