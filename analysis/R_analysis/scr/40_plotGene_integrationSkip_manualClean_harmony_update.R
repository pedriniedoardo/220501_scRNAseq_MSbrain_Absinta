# AIM ---------------------------------------------------------------------
# this is the update for the EDA of TSPO expression
...
# explore expression ------------------------------------------------------
data.combined$group_cellType <- paste0(data.combined$disease,"-",data.combined$origin,"-",data.combined$expertAnno.l1,"-",data.combined$orig.ident)
# data.combined$group2 <- paste0(data.combined$orig.ident,".",data.combined$treat,".",data.combined$cell_type2)
...
df_avg_cellType <- average_group_cellType$RNA %>%
  ...
  mutate(gene = GOI) |> 
  pivot_longer(names_to = "group",values_to = "avg_exp",-gene) %>%
  mutate(disease = str_extract(group,pattern = "CTRL|MS")) %>%
  mutate(expertAnno.l1 = str_remove_all(group,pattern = "CTRL.|MS.|cortex.|wm.|.s\\d+")) %>%
  mutate(origin = str_extract(group,pattern = "cortex|wm")) %>%
  mutate(sample = str_extract(group,pattern = "s\\d+"))

# save the table
df_avg_cellType %>%
  write_tsv("../../out/table/revision/df_avg_cellType_TSPO_update.tsv")
