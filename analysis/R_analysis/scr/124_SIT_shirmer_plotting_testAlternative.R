...
#   left_join(data.combined@reductions$curated@cell.embeddings %>%
#               data.frame() %>%
...
# enumeration_cellID_ref <- crossing(id = unique(paste(meta_full$sample,meta_full$cellid,meta_full$pathology,sep = "|")),
#                                    SENEQUANTILE = unique(meta_full$SENEQUANTILE)) %>%
#   separate(id,into = c("sample","cellid","pathology"),sep = "\\|")
# 
# enumeration_cellID <- meta_full %>%
#   group_by(pathology,sample,cellid,SENEQUANTILE) %>%
#   summarise(n = n())
# 
# enumeration_cellID_full <- left_join(enumeration_cellID_ref,enumeration_cellID,by = c("sample","cellid","pathology","SENEQUANTILE")) %>%
#   # fill the NA with 0
#   mutate(n = case_when(is.na(n)~0,
#                        T~n)) |> 
#   group_by(pathology,cellid) %>%
#   mutate(tot = sum(n)) %>%
#   ungroup() %>%
#   mutate(prop = n/tot) %>%
