...
ggsave("../../out/image/129_UMAP_SigAbsinta2019_IMMLYMsubset_highres.pdf",width = 25,height = 20)

# same as above but as violin plot
list_plot <- lapply(df_rename_long$rename, function(x){ 
  test <- VlnPlot(object = data.combined,features = x, group.by = "RNA_snn_res.0.9",raster = T)
  return(test)
})
...
df_senescence <- data.combined@meta.data %>%
  dplyr::select(RNA_snn_res.0.9,contains("score_")) %>%
  pivot_longer(names_to = "signature",values_to = "score",-RNA_snn_res.0.9) %>%
  group_by(RNA_snn_res.0.9,signature) %>%
...
mat_senescence_avg %>%
  ...
  write_tsv("../../out/table/129_tableScaled_SigAbisnta2019_res0.9_IMMLYMsubset_highres.tsv")

df_senescence %>%
  write_tsv("../../out/table/129_tableUnScaled_SigAbisnta2019_res0.9_IMMLYMsubset_highres.tsv")

pdf("../../out/image/129_heatmap_SigAbisnta2019_res0.9_IMMLYMsubset_highres.pdf",width = 12,height = 5)
...
pdf("../../out/image/129_heatmap_SigAbisnta2019_res0.9_IMMLYMsubset_highres2.pdf",width = 12,height = 5)
