...
df_meta_full %>%
  ...
  ungroup() %>%
  mutate(RNA_snn_res.0.5 = fct_reorder(RNA_snn_res.0.5,-avg_prop)) %>%
  ggplot(aes(x=RNA_snn_res.0.5,y=prop,fill=var))+geom_col(position = "dodge")+theme_cowplot()
...
df_meta_full %>%
  ...
  ungroup() %>%
  mutate(RNA_snn_res.0.5 = fct_reorder(RNA_snn_res.0.5,-avg_prop)) %>%
  write_tsv("../../out/table/131_prop_res0.5_senescence_AST.tsv")

# add the sample info
df_meta_full %>%
  group_by(orig.ident,pathology_class,RNA_snn_res.0.5) %>%
  summarise(n = n(),
            tot_sen_SIT = sum(sen_SIT),
            tot_sen_SENMAYO = sum(sen_SENMAYO)) %>%
  ungroup() %>%
  ...
  ungroup() %>%
  mutate(RNA_snn_res.0.5 = fct_reorder(RNA_snn_res.0.5,-avg_prop)) %>%
  ggplot(aes(x=RNA_snn_res.0.5,y=prop)) +
...
df_meta_full %>%
  ...
  ungroup() %>%
  mutate(RNA_snn_res.0.5 = fct_reorder(RNA_snn_res.0.5,-avg_prop)) %>%
  ggplot(aes(x=RNA_snn_res.0.5,y=prop,col=var)) +
  geom_boxplot(position = "dodge",outlier.shape = NA) +
...
df_meta_full %>%
  ...
  ungroup() %>%
  mutate(RNA_snn_res.0.5 = fct_reorder(RNA_snn_res.0.5,-avg_prop)) %>%
  write_tsv("../../out/table/131_prop_res0.5_senescence_sample_AST.tsv")
...
df_meta_full %>%
  ...
  mutate(RNA_snn_res.0.5 = fct_reorder(RNA_snn_res.0.5,prop)) %>%
...
df_meta_full %>%
  ...
  mutate(prop = n_cluster/n_tot_sample) %>%
  mutate(RNA_snn_res.0.5 = fct_reorder(RNA_snn_res.0.5,prop)) %>%
  write_tsv("../../out/table/131_prop_res0.5_AST.tsv")

# split by origin and disease per cluster
df_meta_full %>%
  group_by(orig.ident,origin,disease,RNA_snn_res.0.5) %>%
  summarise(n_cluster = n()) %>%
  ungroup() %>%
  group_by(orig.ident) %>%
  mutate(n_tot_sample = sum(n_cluster)) %>%
...
df_meta_full %>%
  ...
  mutate(prop = n_cluster/n_tot_sample) %>%
  mutate(RNA_snn_res.0.5 = fct_reorder(RNA_snn_res.0.5,prop)) %>%
  write_tsv("../../out/table/131_prop_res0.5_AST_originDisease.tsv")
...
df_meta_full %>%
  ...
  complete(orig.ident,RNA_snn_res.0.5, fill = list(n_cluster = 0)) %>%
...
df_meta_full %>%
  ...
  mutate(prop = n_cluster/n_tot_sample) %>%
  mutate(RNA_snn_res.0.5 = fct_reorder(RNA_snn_res.0.5,prop)) %>%
  write_tsv("../../out/table/131_prop_res0.5_AST_includeZeros.tsv")
...
df_meta_full %>%
  ...
  mutate(prop = n_cluster/n_tot_sample) %>%
  mutate(RNA_snn_res.0.5 = fct_reorder(RNA_snn_res.0.5,prop)) %>%
  write_tsv("../../out/table/131_prop_res0.5_AST_originDisease_includeZeros.tsv")
