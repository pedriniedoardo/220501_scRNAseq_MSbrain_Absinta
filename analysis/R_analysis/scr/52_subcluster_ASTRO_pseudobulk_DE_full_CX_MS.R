...
# 1. Get counts matrix
counts_ASTRO_Ctrl <- cts_sample_all$RNA %>%
  data.frame() %>%
  rownames_to_column("gene") %>%
  # pull only the ASTRO cells
  dplyr::select("gene",contains("ASTRO")) %>%
  # focus only of the ASTRO from ctrl samples
  dplyr::select("gene",contains("Ctrl")) %>%
  # remove the unassigned donors
  dplyr::select(-contains("unassigned")) %>%
  column_to_rownames("gene")
#
counts_ASTRO_Mye <- cts_sample_all$RNA %>%
  data.frame() %>%
  rownames_to_column("gene") %>%
  # pull only the ASTRO cells
  dplyr::select("gene",contains("ASTRO")) %>%
  ...
  column_to_rownames("gene")
#
# counts_ASTRO_NAWM <- cts_sample_all$RNA %>%
#   data.frame() %>%
#   rownames_to_column("gene") %>%
#   # pull only the ASTRO cells
#   dplyr::select("gene",contains("ASTRO")) %>%
...
identical(rownames(counts_ASTRO_Ctrl),rownames(counts_ASTRO_Mye))
# identical(rownames(counts_ASTRO_Mye),rownames(counts_ASTRO_NAWM))

# merge the three tables
# filter only for the CX samples
counts_ASTRO <- bind_cols(counts_ASTRO_Ctrl,counts_ASTRO_Mye) %>%
  select(contains("CX_"))
...
saveRDS(counts_ASTRO,file = "../../out/object/counts_ASTRO_full_CX.rds")
saveRDS(colData_ASTRO,file = "../../out/object/colData_ASTRO_full_CX.rds")

# perform DESeq2 ----------------------------------------------------------
# build the model
# location_full <- colData_ASTRO$origin_01
treat_full <- colData_ASTRO$disease
# design_ASTRO <- model.matrix(~ location_full+treat_full)
design_ASTRO <- model.matrix(~ treat_full)
colnames(design_ASTRO)[1] <- c("intercept")
# colnames(design_ASTRO)[4] <- c("location_fullWM.treat_fullMS")
# save the disign
saveRDS(design_ASTRO,"../../out/object/design_ASTRO_full_CX.rds")
...
pdf("../../out/image/heatmap_SampleCluster_pseudobulk_ASTRO_full_CX.pdf",width = 10,height = 6)
set.seed(3)
...
# ggsave("../../out/image/PCA_pseudobulk_ASTRO_ctrl_refCX_test.pdf",width = 6,height = 4)
...
plot_vsd_ASTRO$data %>%
  ...
  scale_x_continuous(expand = expansion(mult = 0.1))+
  theme_bw() + ylab(plot_vsd_ASTRO$labels[1]) + xlab(plot_vsd_ASTRO$labels[2])
ggsave("../../out/image/PCA_pseudobulk_ASTRO_full_CX.pdf",width = 6,height = 4)
...
pdf("../../out/image/heatmap_cellType_pseudobulk_ASTRO_full_CX.pdf",width = 7,height = 5)
...
ht2_shr_ASTRO22 <- Heatmap(sample_prop_wide2,
                           show_column_names = T,
                           raster_by_magick = T,
...
pdf("../../out/image/heatmap_cellType_pseudobulk_ASTRO_full_CX2.pdf",width = 7,height = 5)
draw(ht2_shr_ASTRO22,heatmap_legend_side = "left",annotation_legend_side = "left",padding = unit(c(30,2,2, 2), "mm"))
dev.off()
...
saveRDS(ddsHTSeq_ASTRO_filter,"../../out/object/ddsHTSeq_pseudobulk_ASTRO_full_CX.rds")
...
contrast_ASTRO <- makeContrasts(ASTRO_MS_vs_CTRL = treat_fullMS,
                                # ASTRO_WM_MS_int = location_fullWM.treat_fullMS,
                                levels = design_ASTRO)

# pull the results table
res_ASTRO.MS <- results(ddsHTSeq_ASTRO_filter, contrast=contrast_ASTRO[,"ASTRO_MS_vs_CTRL"],alpha = 0.05)
# res_ASTRO.WM.MS_int <- results(ddsHTSeq_ASTRO_filter, contrast=contrast_ASTRO[,"ASTRO_WM_MS_int"],alpha = 0.05)
summary(res_ASTRO.MS)
# summary(res_ASTRO.WM.MS_int)

# add the gene symbols
df_res <-
  list(ASTRO.CX.MS = res_ASTRO.MS) %>%
  lapply(function(x){
    df <- x %>%
      data.frame() %>%
...
# save the table
df_res %>%
  write_tsv("../../out/table/DE_pseudobulk_ASTRO_full_CX.tsv")

# shrink ------------------------------------------------------------------
res_ASTRO.MS_shr <- lfcShrink(ddsHTSeq_ASTRO_filter, res = res_ASTRO.MS, type = "ashr")

# save the table of DEGs
df_res_shr <-
  list(ASTRO.CX.MS_shr = res_ASTRO.MS_shr) %>%
  lapply(function(x){
    df <- x %>%
      data.frame() %>%
      rownames_to_column(var = "symbol") %>%
...
# save the table
df_res_shr %>%
  write_tsv("../../out/table/DE_pseudobulk_ASTRO_full_CX_shr.tsv")
...
ggsave("../../out/image/histogram_pvalue_pseudobulk_ASTRO_full_CX.pdf",width = 6,height = 4)
...
ggsave("../../out/image/vulcano_plot_pseudobulk_ASTRO_full_CX.pdf",width = 10,height = 10)
...
#   theme(strip.background = element_blank())+
#   theme(legend.position = "none")
# ggsave("../../out/image/vulcano_plot_pseudobulk_ASTRO_ctrl_refCX_shr.pdf",width = 10,height = 10)
...
#   theme(strip.background = element_blank()) +
#   theme(legend.position = "none")
# ggsave("../../out/image/MA_plot_pseudobulk_ASTRO_ctrl_refCX_test.pdf",width = 10,height = 10)
...
#   theme(strip.background = element_blank()) +
#   theme(legend.position = "none")
# ggsave("../../out/image/MA_plot_pseudobulk_ASTRO_ctrl_refCX_shr_test.pdf",width = 10,height = 10)
...
column_ha_shr_ASTRO <- HeatmapAnnotation(disease = meta_sample_ASTRO$disease,
                                         gender = meta_sample_ASTRO$sex,
                                         origin = meta_sample_ASTRO$origin_01,
...
ht2_shr_ASTRO <- Heatmap(mat2_shr_ASTRO, show_column_names = T,raster_by_magick = T,show_row_dend = F, use_raster = T,
                         ...
                         # row_names_gp = gpar(fontsize = 3),
                         top_annotation = column_ha_shr_ASTRO,show_row_names = T,
                         # cluster_rows = F,
                         # right_annotation = row_ha,
                         # row_split = rep(c(1,2,3,4),c(2,3,4,7))
                         
)
pdf("../../out/image/heatmap_DEG_plot_pseudobulk_ASTRO_full_CX_shr.pdf",width = 7,height = 7)
...
mat_shr_ASTRO2 <- mat_filter_ASTRO[rownames(vds_ASTRO_filter) %in% DEG_3, ]
mat2_shr_ASTRO2 <- (mat_shr_ASTRO2 - rowMeans(mat_shr_ASTRO2))/rowSds(mat_shr_ASTRO2,useNames = TRUE)
#
meta_sample_ASTRO2 <- data.frame(colname = colnames(mat2_shr_ASTRO2)) %>%
  left_join(colData_ASTRO,by=c("colname"="samples"))

# make the column of the matrix more readable
colnames(mat2_shr_ASTRO2) <- meta_sample_ASTRO$colname

column_ha_shr_ASTRO2 <- HeatmapAnnotation(disease = meta_sample_ASTRO2$disease,
                                         gender = meta_sample_ASTRO2$sex,
                                         origin = meta_sample_ASTRO2$origin_01,
                                         col = list(origin = c("CX" = "yellow",
                                                               "WM" = "blue"),
                                                    disease = c("MS" = "red",
                                                                "CTRL" = "green"),
...
ht2_shr_ASTRO2 <- Heatmap(mat2_shr_ASTRO2, show_column_names = T,raster_by_magick = T,show_row_dend = F, use_raster = T,
                          name = "exp",
                          column_title = "ASTRO",
                          # row_names_gp = gpar(fontsize = 3),
                          top_annotation = column_ha_shr_ASTRO2,show_row_names = T,
                          # cluster_rows = F,
                          # right_annotation = row_ha,
                          # row_split = rep(c(1,2,3,4),c(2,3,4,7))
                          
)
pdf("../../out/image/heatmap_DEG_plot_pseudobulk_ASTRO_full_CX.pdf",width = 7,height = 10)
draw(ht2_shr_ASTRO2,heatmap_legend_side = "left",annotation_legend_side = "left",padding = unit(c(30,2,2, 2), "mm"))
dev.off()
...
# # PLOT DISPERSION ---------------------------------------------------------
# pdf("../../out/image/dispersion_plot_pseudobulk_ASTRO_ctrl_refCX_test.pdf",width = 5,height = 5)
