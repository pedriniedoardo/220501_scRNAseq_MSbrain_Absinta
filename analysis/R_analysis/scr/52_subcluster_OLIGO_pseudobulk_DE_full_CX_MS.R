# info --------------------------------------------------------------------
# run the comparison for MS (Mye) vs CTRL in CX sample
...
#   rownames_to_column("gene") %>%
#   # pull only the OLIGO cells
#   dplyr::select("gene",contains("OLIGO")) %>%
#   # focus only of the OLIGO from ctrl samples
...
counts_OLIGO <- bind_cols(counts_OLIGO_Ctrl,counts_OLIGO_Mye) %>%
  select(contains("CX_"))
...
saveRDS(counts_OLIGO,file = "../../out/object/counts_OLIGO_full_CX.rds")
saveRDS(colData_OLIGO,file = "../../out/object/colData_OLIGO_full_CX.rds")
...
design_OLIGO <- model.matrix(~ treat_full)
colnames(design_OLIGO)[1] <- c("intercept")
# colnames(design_OLIGO)[4] <- c("location_fullWM.treat_fullMS")
# save the disign
saveRDS(design_OLIGO,"../../out/object/design_OLIGO_full_CX.rds")
...
pdf("../../out/image/heatmap_SampleCluster_pseudobulk_OLIGO_full_CX.pdf",width = 10,height = 6)
set.seed(3)
...
# ggsave("../../out/image/PCA_pseudobulk_OLIGO_ctrl_refCX_test.pdf",width = 6,height = 4)
...
plot_vsd_OLIGO$data %>%
  ...
  scale_x_continuous(expand = expansion(mult = 0.1))+
  theme_bw() + ylab(plot_vsd_OLIGO$labels[1]) + xlab(plot_vsd_OLIGO$labels[2])
ggsave("../../out/image/PCA_pseudobulk_OLIGO_full_CX.pdf",width = 6,height = 4)
...
pdf("../../out/image/heatmap_cellType_pseudobulk_OLIGO_full_CX.pdf",width = 7,height = 5)
...
scobj_subset <- readRDS("../../out/object/ManualClean/data.combined_WM_CX_OLIGO_harmonySkipIntegration.rds")
...
scobj_subset@meta.data %>%
  filter(pathology_class %in% c("CX_Ctrl","CX_Mye")) %>%
...
ht2_shr_OLIGO22 <- Heatmap(sample_prop_wide2,
                        show_column_names = T,
                        raster_by_magick = T,
...
pdf("../../out/image/heatmap_cellType_pseudobulk_OLIGO_full_CX2.pdf",width = 7,height = 5)
draw(ht2_shr_OLIGO22,heatmap_legend_side = "left",annotation_legend_side = "left",padding = unit(c(30,2,2, 2), "mm"))
dev.off()
...
saveRDS(ddsHTSeq_OLIGO_filter,"../../out/object/ddsHTSeq_pseudobulk_OLIGO_full_CX.rds")
...
contrast_OLIGO <- makeContrasts(OLIGO_MS_vs_CTRL = treat_fullMS,
...
# save the table
df_res %>%
  write_tsv("../../out/table/DE_pseudobulk_OLIGO_full_CX.tsv")
...
df_res_shr <-
  list(OLIGO.CX.MS_shr = res_OLIGO.MS_shr) %>%
  lapply(function(x){
    df <- x %>%
      data.frame() %>%
      rownames_to_column(var = "symbol") %>%
...
# save the table
df_res_shr %>%
  write_tsv("../../out/table/DE_pseudobulk_OLIGO_full_CX_shr.tsv")
...
ggsave("../../out/image/histogram_pvalue_pseudobulk_OLIGO_full_CX.pdf",width = 6,height = 4)
...
plot_volcano %>%
  ...
  ggrepel::geom_text_repel(
    data = plot_volcano %>% group_by(conditionVsCTRL) %>% arrange(padj) %>% dplyr::slice(1:30),
    aes(label = symbol),segment.alpha=0.4) +
  facet_wrap(~conditionVsCTRL)+
  theme_bw()+
  theme(strip.background = element_blank())+
  theme(legend.position = "none")
ggsave("../../out/image/vulcano_plot_pseudobulk_OLIGO_full_CX.pdf",width = 10,height = 10)
...
column_ha_shr_OLIGO <- HeatmapAnnotation(disease = meta_sample_OLIGO$disease,
                                         gender = meta_sample_OLIGO$sex,
                                         origin = meta_sample_OLIGO$origin_01,
                                         col = list(origin = c("CX" = "yellow",
                                                               "WM" = "blue"),
                                                    disease = c("MS" = "red",
                                                                "CTRL" = "green"),
                                                    gender = c("M" = "cyan",
...
ht2_shr_OLIGO <- Heatmap(mat2_shr_OLIGO, show_column_names = T,raster_by_magick = T,show_row_dend = F, use_raster = T,
                      ...
                      # row_names_gp = gpar(fontsize = 3),
                      top_annotation = column_ha_shr_OLIGO,show_row_names = T,
                      # cluster_rows = F,
                      # right_annotation = row_ha,
                      # row_split = rep(c(1,2,3,4),c(2,3,4,7))
                      
)
pdf("../../out/image/heatmap_DEG_plot_pseudobulk_OLIGO_full_CX_shr.pdf",width = 7,height = 18)
...
# make the column of the matrix more readable
colnames(mat2_shr_OLIGO2) <- meta_sample_OLIGO$colname

column_ha_shr_OLIGO2 <- HeatmapAnnotation(disease = meta_sample_OLIGO2$disease,
                                          gender = meta_sample_OLIGO2$sex,
                                          origin = meta_sample_OLIGO2$origin_01,
                                          col = list(origin = c("CX" = "yellow",
                                                                "WM" = "blue"),
                                                     disease = c("MS" = "red",
                                                                 "CTRL" = "green"),
...
ht2_shr_OLIGO2 <- Heatmap(mat2_shr_OLIGO2, show_column_names = T,raster_by_magick = T,show_row_dend = F, use_raster = T,
                       ...
                       column_title = "OLIGO",
                       # row_names_gp = gpar(fontsize = 3),
                       top_annotation = column_ha_shr_OLIGO2,show_row_names = T,
                       # cluster_rows = F,
                       # right_annotation = row_ha,
                       # row_split = rep(c(1,2,3,4),c(2,3,4,7))
                       
)
pdf("../../out/image/heatmap_DEG_plot_pseudobulk_OLIGO_full_CX.pdf",width = 7,height = 18)
draw(ht2_shr_OLIGO2,heatmap_legend_side = "left",annotation_legend_side = "left",padding = unit(c(30,2,2, 2), "mm"))
dev.off()
...
# # PLOT DISPERSION ---------------------------------------------------------
# pdf("../../out/image/dispersion_plot_pseudobulk_OLIGO_ctrl_refCX_test.pdf",width = 5,height = 5)
