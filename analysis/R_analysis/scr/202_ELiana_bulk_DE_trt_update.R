...
# read in the data --------------------------------------------------------
ddsHTSeq_filter <- readRDS("../../out/object/202_dds_trt_filter_update.rds")
design <- readRDS("../../out/object/201_design_trt_update.rds")
vds_filter <- readRDS("../../out/object/vds_trt_filter_update.rds")
...
saveRDS(ddsHTSeq_filter,"../../out/object/202_dds_filter_trt_UPDATE_DESeq.rds")
...
contrast <- makeContrasts(TRT_vs_NAIVE = treatTRT,
                          levels = design)

res_TRT <- results(ddsHTSeq_filter, contrast=contrast[,"TRT_vs_NAIVE"],alpha = 0.05)
...
# shrink ------------------------------------------------------------------  
res_TRT_shr <- lfcShrink(ddsHTSeq_filter, res = res_TRT, type = "ashr")

summary(res_TRT_shr)

list_df_shr <- 
  list("TRT_shr" = res_TRT_shr) %>%
  lapply(function(x){
    df <- x %>%
      data.frame() %>%
      rownames_to_column(var = "symbol") %>%
...
ggsave("../../out/image/202_histogram_trt_pvalue.pdf",width = 6,height = 3)
...
ggsave("../../out/image/202_vulcano_plot_trt_text.pdf",width = 12,height = 12)
#
...
test_plot_shr %>%
  ...
  facet_wrap(~comparison)+
  theme(legend.position = "none",strip.background = element_blank())
ggsave("../../out/image/202_vulcano_plot_trt_text_shr.pdf",width = 12,height = 12)
...
ggsave("../../out/image/202_MA_plot_trt.pdf",width = 10,height = 3)
...
bind_rows(list_df_shr,.id = "comparison") %>%
  ...
  theme_bw() +
  theme(legend.position = "none",strip.background = element_blank())+facet_wrap(~comparison)
ggsave("../../out/image/202_MA_plot_trt_shr.pdf",width = 24,height = 12)
...
# -------------------------------------------------------------------------
# among the DEGs try to check if there is any gender related genes
list_df_shr$TRT_shr %>%
  filter(symbol %in% c("XIST","DDX3Y","USP9Y","EIF1AY"))

# plot the DEGs for Veh20Dynes --------------------------------------------
DEG_TRT <- list_df_shr$TRT_shr %>%
  data.frame()%>%
  # add a clor variable in case significant
  mutate(col=ifelse(((padj<0.05)&abs(log2FoldChange)>1),yes = 1,no = 0)) %>%
  dplyr::filter(col==1) %>%
...
mat_shr <- mat_filter[rownames(vds_filter) %in% DEG_TRT, ]
# pull only the samples of interested based on the comparison
id_sample <- str_detect(vds_filter@colData$Treatment,pattern = "Naive|TRT")
...
pdf("../../out/image/202_heatmap_DEG_shr_TRT.pdf",width = 5,height = 6) 
draw(ht2_shr,heatmap_legend_side = "left",annotation_legend_side = "left") 
dev.off()


# PLOT DISPERSION ---------------------------------------------------------
pdf("../../out/image/202_ddsHTSeq_filter_trt_dispersion.pdf",width = 5,height = 5) 
