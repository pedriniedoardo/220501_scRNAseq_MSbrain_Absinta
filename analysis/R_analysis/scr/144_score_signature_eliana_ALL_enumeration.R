...
list_enumeration <- lapply(id_signature,function(x){
  ...
  df_en_090_1 <- df_modules %>% 
    dplyr::select(barcode,signature,test,expertAnno.l1,signature_score1) %>% 
    filter(signature %in% x) %>% 
    left_join(df_90,by = c("expertAnno.l1","signature")) %>% 
    mutate(pass_thr = signature_score1>thr) %>% 
    group_by(signature,expertAnno.l1,test) %>% 
  ...
  df_en_090_2 <- df_en_090_1 %>% 
    filter(test %in% "cortex_CTRL") %>% 
    dplyr::select(signature,expertAnno.l1,prop_sen_ctrl=prop_sen)
...
# save the table
df_en_090_final %>% 
  write_tsv("../../out/table/144_enumeration_ALL_090_threshold_custom.tsv")
...
ggsave(paste0("../../out/image/144_enumeration_log2Prop_ALL_090_threshold_custom.pdf"),width = 12,height =8)
...
list_enumeration_cellID <- lapply(id_signature,function(x){
  ...
  test_summary_cellID <- df_modules_cellID %>% 
    ...
    mutate(sen = case_when(signature_score1>thr~1,
                           T~0)) %>% 
    group_by(signature,orig.ident,test,expertAnno.l1,sen) %>% 
    summarise(n = n()) %>% 
    ungroup() %>% 
    group_by(signature,orig.ident,test,expertAnno.l1) %>% 
    mutate(tot = sum(n)) %>% 
    ungroup() %>% 
    mutate(prop = n/tot)
...
# save the table
enumeration_cellID %>% 
  write_tsv("../../out/table/144_enumeration_ALL_090_threshold_sampleWise_custom.tsv")
...
ggsave("../../out/image/144_enumeration_log2Prop_ALL_090_threshold_above5_sampleWise_custom.pdf",width = 13,height = 8)
...
list_enumeration_cellID_zeros <- lapply(id_signature,function(x){
  ...
  tot_cells <- df_modules_cellID %>% 
    filter(signature %in% x) %>% 
    group_by(signature,orig.ident,expertAnno.l1) %>% 
    summarise(n = n(),.groups = "drop") %>%
    complete(signature,orig.ident,expertAnno.l1, fill = list(n = 0))
  ...
  tot_cells_senescence <- df_modules_cellID %>% 
    ...
    complete(signature,orig.ident,expertAnno.l1, fill = list(n_sen = 0))
  ...
  test_summary_cellID <- tot_cells %>%
    left_join(tot_cells_senescence,by = c("signature","orig.ident","expertAnno.l1")) %>%
...
enumeration_cellID_zeroes %>% 
  write_tsv("../../out/table/144_enumeration_ALL_090_threshold_sampleWise_custom_zeroes.tsv")
...
enumeration_cellID_zeroes %>%
  ...
  theme(strip.background = element_blank(),
        strip.text.y.right = element_text(angle = 0))+
  geom_hline(yintercept = 0.1,linetype="dotted",col="black")+theme(axis.text.x = element_text(hjust = 1,angle = 90))
ggsave("../../out/image/144_enumeration_log2Prop_ALL_090_threshold_above5_sampleWise_custom_zeroes.pdf",width = 13,height =8)
