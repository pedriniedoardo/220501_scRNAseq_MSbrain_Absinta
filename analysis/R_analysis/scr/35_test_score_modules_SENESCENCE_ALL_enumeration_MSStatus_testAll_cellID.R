...
df_modules <- lapply(file, function(x){
  ...
  mutate(cell_id = case_when(cell_id %in% c(0,4,14)~"OLIGO",
                             cell_id %in% c(9)~"OPC",
                             cell_id %in% c(3,12)~"ASTRO",
                             cell_id %in% c(5)~"IMMUNE",
                             cell_id %in% c(13)~"LYM",
                             cell_id %in% c(11)~"VAS",
                             cell_id %in% c(1, 2, 10,6)~"EXC NEU",
                             cell_id %in% c(7,8)~"INH NEU"))
...
list_enumeration <- lapply(id_signature,function(x){
  ...
  df_en_090_1 <- df_modules %>% 
    ...
    filter(signature %in% x) %>% 
    left_join(df_90,by = c("cell_id","signature")) %>% 
    mutate(pass_thr = signature_score1>thr) %>% 
    group_by(signature,cell_id,disease,origin) %>% 
  ...
  df_en_090_2 <- df_en_090_1 %>% 
    filter(disease %in% "CTRL",
           origin == "cortex") %>% 
    select(signature,cell_id,prop_sen_ctrl=prop_sen)
  
  df_en_090_final <- df_en_090_1 %>% 
    left_join(df_en_090_2,by = c("cell_id","signature")) %>% 
...
ggsave(paste0("../../out/image/ManualClean/modules_SENESCENCE/enumeration_log2Prop_ALL_090_threshold_above5_MSStatus_CXref_cellID.pdf"),width = 18,height =20)
...
df_en_090_final %>% 
  ...
  theme(strip.background = element_blank(), 
        ...
        strip.text.y.right = element_text(angle = 0))+
  scale_fill_gradientn(colours = viridis::turbo(10),trans = "log",breaks = 4^seq(1,5))+geom_vline(xintercept = 0,linetype="dashed",col="gray")
ggsave(paste0("../../out/image/ManualClean/modules_SENESCENCE/enumeration_log2Prop_ALL_090_threshold_above5_MSStatus_CXref_cellID_small.pdf"),width = 18,height =10)
...
# save the table
enumeration_cellID %>% 
  write_tsv("../../out/table/ManualClean/modules_SENESCENCE/HVG4000/enumeration_ALL_090_threshold_MSStatus_refCX_cellID_sampleWise.tsv")
...
ggsave(paste0("../../out/image/ManualClean/modules_SENESCENCE/enumeration_log2Prop_ALL_090_threshold_above5_MSStatus_CXref_cellID_sampleWise.pdf"),width = 18,height =20)
...
ggsave(paste0("../../out/image/ManualClean/modules_SENESCENCE/enumeration_log2Prop_ALL_090_threshold_above5_MSStatus_CXref_cellID_sampleWise_small.pdf"),width = 18,height =10)
...
enumeration_cellID %>%
  ...
  geom_point(position = position_jitter(width = 0.1)) +
  # geom_text_repel()+
  facet_wrap(signature~cell_id,scales = "free",ncol = 8)+
  theme_bw()+
  theme(strip.background = element_blank(),
        strip.text.y.right = element_text(angle = 0))+
  geom_hline(yintercept = 0.1,linetype="dotted",col="black")+theme(axis.text.x = element_text(hjust = 1,angle = 90))
ggsave(paste0("../../out/image/ManualClean/modules_SENESCENCE/enumeration_log2Prop_ALL_090_threshold_above5_MSStatus_CXref_cellID_sampleWise_small2.pdf"),width = 18,height =10)
