...
data <- readRDS("../../out/object/201_dds_trt_update.rds") %>%
  DESeq()
# data <- readRDS("../../out/object/202_dds_all_filter_update.rds") %>%
#   DESeq()

# pull the list of degs reported by eliana
test_DEGS <- readxl::read_xlsx("../../data/signatures/eliana_endo_ms_update/DEGs_TRT_ctrl.xlsx")
...
# old leading edges REACTOME_OTHER_INTERLEUKIN_SIGNALING
GOI <- c("STX1A","VAMP2","JAK1","STX3","STX4")

# REACTOME_REGULATION_OF_TP53_ACTIVITY_THROUGH_PHOSPHORYLATION
GOI <- c("AURKB","CCNA2","EXO1","BARD1","BRIP1","BLM","DNA2","TPX2","RMI2","BRCA1","RFC5","RMI1","RBBP8","RFC4","AURKA","RPA3","RFC3")
...
counts(data,normalized=T) %>%
  ...
  ggplot(aes(x=Treatment,y = count_norm_adj))+
...
vst <- readRDS(file = "../../out/object/vds_trt_filter_update.rds")
