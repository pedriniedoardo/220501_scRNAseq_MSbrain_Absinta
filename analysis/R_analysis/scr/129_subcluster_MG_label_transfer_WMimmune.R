...
DimPlot(query,label = T,raster = T,group.by = "RNA_snn_res.0.1")
DimPlot(query,label = T,raster = T,group.by = "RNA_snn_res.0.1",split.by = "pathology_class")

# fix the meta to add the general cell annotation
query$seurat_clusters_fix1 <- paste0("cluster_",query$RNA_snn_res.0.1)
...
ggsave("../../out/image/129_histo_label_transfer_refWMImmune_queryIMMLYMsubset.pdf",width = 3,height = 2)
...
ggsave("../../out/image/129_histo_label_transfer_refWMImmune_queryIMMLYMsubset_split.pdf",width = 9,height = 6)
...
pdf("../../out/image/129_heatmap_label_transfer_refWMImmune_queryIMMLYMsubset_raw.pdf",height = 4,width = 4)
...
# save the object
saveRDS(query_transfer,"../../out/object/129_query_transfer_WM_Immune.rds")
...
ggsave("../../out/image/129_UMAP_label_transfer_refWMImmune_queryIMMLYMsubset_raw_inverse.pdf",width = 6,height = 4)
...
ggsave("../../out/image/129_UMAP_label_transfer_refWMImmune_queryIMMLYMsubset_raw_inverse2.pdf",width = 18,height = 12)
...
ggplot(label= F)+
  ...
  guides(colour = guide_legend(override.aes = list(size=5),ncol = 1))+
  theme(legend.position = "none")
ggsave("../../out/image/129_UMAP_label_transfer_refWMImmune_queryIMMLYMsubset_raw_inverse_no_lab.pdf",width = 4,height = 4)
...
ggplot(label= TRUE) +
  ...
  guides(colour = guide_legend(override.aes = list(size=5),ncol = 1))
ggsave("../../out/image/129_UMAP_label_transfer_refWMImmune_queryIMMLYMsubset_robust_inverse.pdf",width = 6,height = 4)
...
ggplot(label= TRUE) +
  ...
  ggrepel::geom_text_repel(data = data2_transfer_avg,aes(x = refUMAP_1,y = refUMAP_2,label = seurat_clusters_fix1),col="black")+theme_cowplot()+
  guides(colour = guide_legend(override.aes = list(size=5),ncol = 1))+facet_wrap(~pathology_class)+theme(strip.background = element_blank())
ggsave("../../out/image/129_UMAP_label_transfer_refWMImmune_queryIMMLYMsubset_robust_inverse2.pdf",width = 18,height = 12)
...
ggplot(label= F) +
  ...
  guides(colour = guide_legend(override.aes = list(size=5),ncol = 1))+
  theme(legend.position = "none")
ggsave("../../out/image/129_UMAP_label_transfer_refWMImmune_queryIMMLYMsubset_robust_inverse_no_lab.pdf",width = 4,height = 4)
