...
counts_ASTRO <- bind_cols(counts_ASTRO_Ctrl,counts_ASTRO_Mye,counts_ASTRO_NAWM)
...
saveRDS(counts_ASTRO,file = "../../out/object/counts_ASTRO_full.rds")
saveRDS(colData_ASTRO,file = "../../out/object/colData_ASTRO_full.rds")
...
# save the disign
saveRDS(design_ASTRO,"../../out/object/design_ASTRO_ctrl_refCX_full.rds")
...
pdf("../../out/image/heatmap_SampleCluster_pseudobulk_ASTRO_ctrl_refCX_full.pdf",width = 10,height = 6)
set.seed(1)
...
ggsave("../../out/image/PCA_pseudobulk_ASTRO_ctrl_refCX_full.pdf",width = 6,height = 4)
...
plot_vsd_ASTRO$data %>%
  ...
  scale_x_continuous(expand = expansion(mult = 0.1))+
  theme_bw() + ylab(plot_vsd_ASTRO$labels[1]) + xlab(plot_vsd_ASTRO$labels[2])
ggsave("../../out/image/PCA_pseudobulk_ASTRO_ctrl_refCX_full2.pdf",width = 6,height = 4)
...
pdf("../../out/image/heatmap_cellType_pseudobulk_ASTRO_ctrl_refCX_full.pdf",width = 7,height = 5)
...
saveRDS(ddsHTSeq_ASTRO_filter,"../../out/object/ddsHTSeq_pseudobulk_ASTRO_ctrl_refCX_full.rds")
...
contrast_ASTRO <- makeContrasts(ASTRO_Ctrl_WM_vs_CX = location_fullWM,
                                ASTRO_MS_vs_CTRL = treat_fullMS,
...
# save the table
df_res %>%
  write_tsv("../../out/table/DE_pseudobulk_ASTRO_ctrl_refCX_cull.tsv")

# add the gene symbols
df_res2 <-
  list(ASTRO.MS = res_ASTRO.MS) %>%
  lapply(function(x){
    df <- x %>%
      data.frame() %>%
      rownames_to_column(var = "symbol") %>%
...
# save the table
df_res2 %>%
  write_tsv("../../out/table/DE_pseudobulk_ASTRO_MS_full.tsv")
...
# save the table
df_res_shr %>%
  write_tsv("../../out/table/DE_pseudobulk_ASTRO_ctrl_refCX_shr_full.tsv")

# save the table of DEGs
df_res_shr2 <-
  list(ASTRO.MS_shr = res_ASTRO.MS_shr) %>%
  lapply(function(x){
    df <- x %>%
      data.frame() %>%
      rownames_to_column(var = "symbol") %>%
...
# save the table
df_res_shr2 %>%
  write_tsv("../../out/table/DE_pseudobulk_ASTRO_MS_shr_full.tsv")
...
ggsave("../../out/image/histogram_pvalue_pseudobulk_ASTRO_ctrl_refCX_full.pdf",width = 6,height = 4)
...
df_res2 %>%
  ...
  theme_bw()+
  theme(strip.background = element_blank())
ggsave("../../out/image/histogram_pvalue_pseudobulk_ASTRO_MS_full.pdf",width = 6,height = 4)
...
ggsave("../../out/image/vulcano_plot_pseudobulk_ASTRO_ctrl_refCX_full.pdf",width = 10,height = 10)
...
plot_volcano2 %>%
  ...
  theme(strip.background = element_blank())+
  theme(legend.position = "none")
ggsave("../../out/image/vulcano_plot_pseudobulk_ASTRO_MS_full.pdf",width = 10,height = 10)
...
column_ha_shr_ASTRO <- HeatmapAnnotation(location = meta_sample_ASTRO$origin_01,
                                         disease = meta_sample_ASTRO$disease,
                                         col = list(location = c("WM" = "blue",
                                                                 "CX" = "yellow"),
                                                    disease = c("CTRL" = "green",
                                                                "MS" = "red")))
...
pdf("../../out/image/heatmap_DEG_plot_pseudobulk_ASTRO_ctrl_refCX_shr_full.pdf",width = 7,height = 14)
...
# make the column of the matrix more readable
colnames(mat2_shr_ASTRO2) <- meta_sample_ASTRO2$colname

column_ha_shr_ASTRO2 <- HeatmapAnnotation(location = meta_sample_ASTRO2$origin_01,
                                          disease = meta_sample_ASTRO2$disease,
                                          col = list(location = c("WM" = "blue",
                                                                  "CX" = "yellow"),
                                                     disease = c("CTRL" = "green",
                                                                 "MS" = "red")))
...
ht2_shr_ASTRO2 <- Heatmap(mat2_shr_ASTRO2, show_column_names = T,raster_by_magick = T,show_row_dend = F, use_raster = T,
                          ...
                          column_title = "ASTRO_shr",
                          # row_names_gp = gpar(fontsize = 3),
                          top_annotation = column_ha_shr_ASTRO2,show_row_names = F,
                          # cluster_rows = F,
                          # right_annotation = row_ha,
                          # row_split = rep(c(1,2,3,4),c(2,3,4,7))
                          
)
pdf("../../out/image/heatmap_DEG_plot_pseudobulk_ASTRO_MS_shr_full.pdf",width = 7,height = 14)
draw(ht2_shr_ASTRO2,heatmap_legend_side = "left",annotation_legend_side = "left",padding = unit(c(30,2,2, 2), "mm"))
dev.off()
...
# PLOT DISPERSION ---------------------------------------------------------
pdf("../../out/image/dispersion_plot_pseudobulk_ASTRO_ctrl_refCX_full.pdf",width = 5,height = 5)
