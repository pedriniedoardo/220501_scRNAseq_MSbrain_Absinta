...
sobj_filter <- readRDS("../../out/object/revision/111_sobj_filter.rds")

DimPlot(sobj_filter,raster = T)
...
sobj_total <- sobj_total %>%
  ...
  FindVariableFeatures(selection.method = "vst", nfeatures = 8000) %>%
...
saveRDS(sobj_total_h,"../../out/object/revision/112_WMCX_ManualClean2_harmonySkipIntegration_AllSoupX_8000.rds")
