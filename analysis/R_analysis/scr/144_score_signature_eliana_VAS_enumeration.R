...
# read in the data --------------------------------------------------------
folder <-  "../../out/table/modules_custom_VAS/"
...
df_modules <- lapply(file, function(x){
  ...
  # focus only on 0 5 9 subclusters
  filter(RNA_snn_res.0.4 %in% c("0","5","9"))
# mutate(test = factor(test,levels = c("cortex_CTRL","CSF.ctrl.24h","CSF.MS.24h","CSF.MS.48h","cytokine","Fe","myelin","TBHP")))
...
list_enumeration <- lapply(id_signature,function(x){
  ...
  df_en_090_1 <- df_modules %>% 
    dplyr::select(barcode,signature,test,RNA_snn_res.0.4,signature_score1) %>% 
    filter(signature %in% x) %>% 
    left_join(df_90,by = c("RNA_snn_res.0.4","signature")) %>% 
    mutate(pass_thr = signature_score1>thr) %>% 
    group_by(signature,RNA_snn_res.0.4,test) %>% 
  ...
  df_en_090_2 <- df_en_090_1 %>% 
    filter(test %in% "cortex_CTRL") %>% 
    dplyr::select(signature,RNA_snn_res.0.4,prop_sen_ctrl=prop_sen)
  
  df_en_090_final <- df_en_090_1 %>% 
    left_join(df_en_090_2,by = c("RNA_snn_res.0.4","signature")) %>% 
...
# save the table
df_en_090_final %>% 
  write_tsv("../../out/table/144_enumeration_VAS_090_threshold_custom.tsv")

# plot all the the enumeration
df_en_090_final %>% 
  filter(tot_sen>5) %>% 
  mutate(origin2 = paste0(test)) %>% 
  ggplot(aes(y=origin2,x=log2FC))+
  geom_col(aes(fill=tot_sen))+
  # geom_col(aes(width = tot_sen))+
  facet_grid(signature~RNA_snn_res.0.4)+
  theme_bw()+
...
ggsave(paste0("../../out/image/144_enumeration_log2Prop_VAS_090_threshold_custom.pdf"),width = 8,height =8)
...
LUT_sample <- df_modules %>% 
  group_by(orig.ident,test) %>% 
  summarise()
...
list_enumeration_cellID <- lapply(id_signature,function(x){
  ...
  test_summary_cellID <- df_modules_cellID %>% 
    filter(signature %in% x) %>% 
    left_join(df_90_cellID2,c("RNA_snn_res.0.4","signature")) %>% 
    mutate(sen = case_when(signature_score1>thr~1,
                           T~0)) %>% 
    group_by(signature,orig.ident,test,RNA_snn_res.0.4,sen) %>% 
    summarise(n = n()) %>% 
    ungroup() %>% 
    group_by(signature,orig.ident,test,RNA_snn_res.0.4) %>% 
    mutate(tot = sum(n)) %>% 
    ungroup() %>% 
    mutate(prop = n/tot)
  ...
  df_plot <- test_summary_cellID %>% 
    left_join(LUT_sample,c("orig.ident","test"))
...
# save the table
enumeration_cellID %>% 
  write_tsv("../../out/table/144_enumeration_VAS_090_threshold_sampleWise_custom.tsv")

enumeration_cellID %>%
  # filter(RNA_snn_res.0.4 %in% c(3,5,11)) %>%
  filter(sen == 1) %>% 
  # ggplot(aes(x=pathology_class,y=prop,col=origin,label=orig.ident)) +
  ggplot(aes(x=test,y=prop)) +
  geom_boxplot(outlier.shape = NA)+
  geom_point(position = position_jitter(width = 0.1),shape = 1) +
  # geom_text_repel()+
  facet_grid(signature~RNA_snn_res.0.4,scales = "free")+
  theme_bw()+
  theme(strip.background = element_blank(),
        strip.text.y.right = element_text(angle = 0))+
  geom_hline(yintercept = 0.1,linetype="dotted",col="black")+theme(axis.text.x = element_text(hjust = 1,angle = 90))

ggsave("../../out/image/144_enumeration_log2Prop_VAS_090_threshold_above5_sampleWise_custom.pdf",width = 8,height = 8)
...
enumeration_cellID %>%
  ...
  # geom_text_repel()+
  facet_wrap(signature~RNA_snn_res.0.4,scales = "free",ncol = 9)+
  theme_bw()+
  theme(strip.background = element_blank(),
        strip.text.y.right = element_text(angle = 0))+
  geom_hline(yintercept = 0.1,linetype="dotted",col="black")+theme(axis.text.x = element_text(hjust = 1,angle = 90))
...
list_enumeration_cellID_zeros <- lapply(id_signature,function(x){
  ...
  # tot cells, I need to remove the test since test and sample name are nested
  tot_cells <- df_modules_cellID %>% 
    filter(signature %in% x) %>% 
    group_by(signature,orig.ident,RNA_snn_res.0.4) %>% 
    summarise(n = n(),.groups = "drop") %>%
    complete(signature,orig.ident,RNA_snn_res.0.4, fill = list(n = 0))
  
  # confirm the presence of the zeros to the counting
  tot_cells %>% filter(n == 0)
  ...
  tot_cells_senescence <- df_modules_cellID %>% 
    ...
    complete(signature,orig.ident,RNA_snn_res.0.4, fill = list(n_sen = 0))
  
  # confirm the presence of the zeros to the counting
  tot_cells_senescence %>% filter(n_sen == 0)
  
  # join the two tables
  test_summary_cellID <- tot_cells %>%
    left_join(tot_cells_senescence,by = c("signature","orig.ident","RNA_snn_res.0.4")) %>%
    mutate(prop_sen = n_sen/n) %>%
    # add the test back to the table 
...
# 
# tot_cells %>%
#   group_by(signature,orig.ident,test) %>%
#   summarise(n = sum(n)) %>%
#   pivot_wider(names_from = test,values_from = n)
# 
...
enumeration_cellID_zeroes %>% 
  write_tsv("../../out/table/144_enumeration_VAS_090_threshold_sampleWise_custom_zeroes.tsv")
...
enumeration_cellID_zeroes %>%
  ...
  theme(strip.background = element_blank(),
        strip.text.y.right = element_text(angle = 0))+
  geom_hline(yintercept = 0.1,linetype="dotted",col="black")+theme(axis.text.x = element_text(hjust = 1,angle = 90))
ggsave("../../out/image/144_enumeration_log2Prop_VAS_090_threshold_above5_sampleWise_custom_zeroes.pdf",width = 8,height =8)
