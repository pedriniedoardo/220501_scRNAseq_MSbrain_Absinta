...
# this is a task for Eliana's project, she has provided a list of genes that are upregulated or downregulated in her comparisons
# this specific sript I am exploring the signed negative log pvalue ranking
...
results <- read_tsv("../../out/table/145_DE_pseudobulk_filterExp_shr_ENDO.tsv") %>%
  split(f = .$condVsCTRL)
...
# pull the terms and genes provided by Eliana
...
pathways <- split(x = gene_sets$Genes, f = gene_sets$Pathway)
# head(pathways)
...
df_tables_GSEA_all %>%
  write_tsv(paste0("../../out/table/146_df_table_GSEA_custom_rankLogPval_","ENDO",".tsv"))
...
# # save the table
# df_tables_GSEA_all_non_redundant %>%
#   write_tsv(file = paste0("../../out/table/23_df_table_GSEA_GOBP_nonredundant_rankLogPval_","ENDO",".tsv"))
...
# ggsave(paste0("../../out/image/23_GSEA_unbiased_GOBP_nonredundant_rankLogPval_","ENDO",".pdf"),width = 15,height = 15)
...
df_tables_GSEA_all %>%
  ...
  geom_text_repel(size = 2,box.padding = 0.5,segment.alpha = 0.6,max.overlaps = 10)+
  geom_hline(yintercept = -log(0.05),col="gray",linetype="dashed")
# ggsave(paste0("../../out/image/23_GSEA_unbiased_GOBP_rankLogPval_","ENDO",".pdf"),width = 15,height = 15)
...
# library("patchwork")
plot_pathway <- lapply(names(pathways),function(x){
  name <- str_sub(x,start = 1,end = -1)
  # pdf(file = paste0("HUVEC_BMP9_24h_",name,".pdf"),width = 3,height = 3)
  plotEnrichment(pathways[[x]], list_ranks$res_MS) + labs(title = name)
})

wrap_plots(plot_pathway)
ggsave(filename = "../../out/image/146_GSEA_plot_profile_ENDO_shr_custom_rankLogPval.pdf",width = 10,height = 10)
