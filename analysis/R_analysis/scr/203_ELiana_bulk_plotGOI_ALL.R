...
test <- readRDS("../../out/object/201_dds_all.rds")
data <- readRDS("../../out/object/202_dds_all_filter.rds") %>%
  DESeq()


test_DEGS <- readRDS("../../out/object/143_custom_signatures_eliana_endo.rds")

test_DEGS$ECFC_MSvsCTRL_POS %>%
  dplyr::filter(Genes %in% c("ABCA4"))

lut <- colData(data) %>%
  as.data.frame()

# GOI <- c(sig_B$symbol,sig_D$symbol)
GOI <- c("CHST9", "ENSG00000283633", "DNASE2B", "RAMP3", "NRG3", "IRS4-AS1", 
         "PROX1", "EXOC3L1", "NOL4", "DTX4", "TRNP1", "MYO1F", "LEPR", "SYNDIG1", 
         "PADI2", "AFF2", "KLHL4", "SERPINA1", "TPTEP1", "ENSG00000253775", 
         "EPHA7", "NNAT", "L3MBTL4", "EMILIN1", "HERC2P3", "ABCA4", "SLC30A3", 
         "TMPRSS11E", "TNNT1", "HERC2P9", "PDPN", "CRIP1", "RBP1", "POF1B", 
         "CES1P1", "SNCAIP", "ZNF423", "CXCL12", "SLC2A12", "CHDH", "BMX", 
         "IL7", "TRIL")
# GOI <- subset_genes
