...
identical(rownames(counts_MG_Ctrl),rownames(counts_MG_Mye))
identical(rownames(counts_MG_Mye),rownames(counts_MG_NAWM))

# merge the three tables
counts_MG <- bind_cols(counts_MG_Ctrl,counts_MG_Mye,counts_MG_NAWM)
...
saveRDS(counts_MG,file = "../../out/object/counts_MG_test.rds")
saveRDS(colData_MG,file = "../../out/object/colData_MG_test.rds")
...
# save the disign
saveRDS(design_MG,"../../out/object/design_MG_ctrl_refCX_test.rds")
...
pdf("../../out/image/heatmap_SampleCluster_pseudobulk_MG_ctrl_refCX_test.pdf",width = 10,height = 6)
set.seed(1)
...
plot_vsd_MG$data %>%
  ...
  scale_x_continuous(expand = expansion(mult = 0.1))+
  theme_bw() + ylab(plot_vsd_MG$labels[1]) + xlab(plot_vsd_MG$labels[2])
ggsave("../../out/image/PCA_pseudobulk_MG_ctrl_refCX_test2.pdf",width = 6,height = 4)
...
pdf("../../out/image/heatmap_cellType_pseudobulk_MG_ctrl_refCX_test.pdf",width = 7,height = 5)
...
saveRDS(ddsHTSeq_MG_filter,"../../out/object/ddsHTSeq_pseudobulk_MG_ctrl_refCX_test.rds")
...
contrast_MG <- makeContrasts(MG_Ctrl_WM_vs_CX = location_fullWM,
                             MG_MS_vs_CTRL = treat_fullMS,
...
# save the table
df_res %>%
  write_tsv("../../out/table/DE_pseudobulk_MG_ctrl_refCX_test.tsv")

# add the gene symbols
df_res2 <-
  list(MG.MS = res_MG.MS) %>%
  lapply(function(x){
    df <- x %>%
      data.frame() %>%
      rownames_to_column(var = "symbol") %>%
...
# save the table
df_res2 %>%
  write_tsv("../../out/table/DE_pseudobulk_MG_MS_test.tsv")
...
# save the table
df_res_shr %>%
  write_tsv("../../out/table/DE_pseudobulk_MG_ctrl_refCX_shr_test.tsv")

# save the table of DEGs
df_res_shr2 <-
  list(MG.MS_shr = res_MG.MS_shr) %>%
  lapply(function(x){
    df <- x %>%
      data.frame() %>%
      rownames_to_column(var = "symbol") %>%
...
# save the table
df_res_shr2 %>%
  write_tsv("../../out/table/DE_pseudobulk_MG_MS_shr_test.tsv")
...
ggsave("../../out/image/histogram_pvalue_pseudobulk_MG_ctrl_refCX_test.pdf",width = 6,height = 4)
...
df_res2 %>%
  ...
  theme_bw()+
  theme(strip.background = element_blank())
ggsave("../../out/image/histogram_pvalue_pseudobulk_MG_MS_test.pdf",width = 6,height = 4)
...
ggsave("../../out/image/vulcano_plot_pseudobulk_MG_ctrl_refCX_test.pdf",width = 10,height = 10)
...
plot_volcano2 %>%
  ...
  theme(strip.background = element_blank())+
  theme(legend.position = "none")
ggsave("../../out/image/vulcano_plot_pseudobulk_MG_MS_test.pdf",width = 10,height = 10)
...
pdf("../../out/image/heatmap_DEG_plot_pseudobulk_MG_ctrl_refCX_shr_test.pdf",width = 7,height = 14)
...
# make the column of the matrix more readable
colnames(mat2_shr_MG2) <- meta_sample_MG2$colname

column_ha_shr_MG2 <- HeatmapAnnotation(location = meta_sample_MG2$origin_01,
                                       disease = meta_sample_MG2$disease,
                                       col = list(location = c("WM" = "blue",
                                                            "CX" = "yellow"),
                                                  disease = c("CTRL" = "green",
                                                              "MS" = "red")))
...
pdf("../../out/image/heatmap_DEG_plot_pseudobulk_MG_MS_shr_test.pdf",width = 7,height = 14)
draw(ht2_shr_MG2,heatmap_legend_side = "left",annotation_legend_side = "left",padding = unit(c(30,2,2, 2), "mm"))
dev.off()
