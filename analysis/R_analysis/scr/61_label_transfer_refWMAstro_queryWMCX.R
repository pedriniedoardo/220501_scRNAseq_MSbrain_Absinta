...
# read in the data --------------------------------------------------------
# in this case I do not need to correct the cluter identities. I already have the model object which is the correct one to work on
# read in the reference dataset for the leng dataset
# ref_WM_original <- readRDS("../../data/all20_astro.rds")
ref_WM <- readRDS("../../data/all20_astro_model.rds")
...
ref_WM$seurat_clusters_original <- paste0("org_",ref_WM$seurat_clusters)

#
# DimPlot(ref_WM,group.by = "seurat_clusters_original",label=T)
# DimPlot(ref_WM,label=T,split.by = "seurat_clusters_original")
# DimPlot(ref_WM,split.by = "seurat_clusters_original",group.by = "seurat_clusters_original")

# # confirm the depletion of cluster 4 cells
# LUT_clusterID_test <- left_join(
#   ref_WM_original@meta.data %>%
#     rownames_to_column("barcode") %>%
#     dplyr::select(barcode,seurat_clusters_original = seurat_clusters),
#   ref_WM@meta.data %>%
#     rownames_to_column("barcode") %>%
#     dplyr::select(barcode,seurat_clusters_new = seurat_clusters)
#   ,by = "barcode") %>%
...
#          seurat_clusters_original = paste0("org_",seurat_clusters_original))
# 
# mat_test <- LUT_clusterID_test %>%
#   group_by(seurat_clusters_original,seurat_clusters_new) %>%
#   summarise(n = n()) %>%
#   mutate(tot = sum(n)) %>%
#   mutate(prop = n/tot) %>%
#   dplyr::select(seurat_clusters_original,seurat_clusters_new,prop) %>%
#   pivot_wider(names_from = seurat_clusters_original,values_from = prop,values_fill = 0) %>%
#   column_to_rownames("seurat_clusters_new")
# 
# Heatmap(mat_test,
#         name = "prop", 
#         column_title = "barcode renaming",
...
query <- readRDS("../../out/object/ManualClean/data.combined_WM_CX_ASTRO_harmonySkipIntegration.rds")
...
ggsave("../../out/image/histo_prediction_label_transfer_refWMASTRO_queryWMCX.pdf",width = 5,height = 4)
...
ggsave("../../out/image/UMAP_label_transfer_refWMASTRO_queryWMCX_raw.pdf",width = 5,height = 4)
...
ggplot(label= TRUE)+
  ...
  guides(colour = guide_legend(override.aes = list(size=5)))
ggsave("../../out/image/UMAP_label_transfer_refWMASTRO_queryWMCX_robust.pdf",width = 5,height = 4)
...
ggsave("../../out/image/histo_label_transfer_refWMASTRO_queryWMCX.pdf",width = 3,height = 4)
...
query_transfer@meta.data %>%
  ...
  theme_bw() +
  geom_vline(xintercept = 0.70,linetype="dashed",col="red",alpha=0.5)
ggsave("../../out/image/histo_label_transfer_refWMASTRO_queryWMCX2.pdf",width = 3,height = 4)
...
pdf("../../out/image/heatmap_label_transfer_refWMASTRO_queryWMCX_raw.pdf",height = 3,width = 3)
...
pdf("../../out/image/heatmap_label_transfer_refWMASTRO_queryWMCX_robust.pdf",height = 3,width = 3)
...
UMAP_ref_WM$data %>%
  ...
  theme_cowplot()+
  guides(colour = guide_legend(override.aes = list(size=5)))
ggsave("../../out/image/UMAP_ref_WM_ASTRO.pdf",width = 6,height = 4)
...
UMAP_ref_WM$data %>%
  ...
  guides(colour = guide_legend(override.aes = list(size=5)))
ggsave("../../out/image/UMAP_ref_WM_ASTRO_no_lab.pdf",width = 4,height = 4)
...
ggsave("../../out/image/UMAP_label_transfer_refWMASTRO_queryWMCX_raw_inverse.pdf",width = 6,height = 4)
...
ggplot(label= F)+
  ...
  guides(colour = guide_legend(override.aes = list(size=5),ncol = 1))+
  theme(legend.position = "none")
ggsave("../../out/image/UMAP_label_transfer_refWMASTRO_queryWMCX_raw_inverse_no_lab.pdf",width = 5,height = 4)
...
ggplot(label= TRUE) +
  ...
  guides(colour = guide_legend(override.aes = list(size=5),ncol = 1))
ggsave("../../out/image/UMAP_label_transfer_refWMASTRO_queryWMCX_robust_inverse.pdf",width = 6,height = 4)
...
ggplot(label= F) +
  ...
  guides(colour = guide_legend(override.aes = list(size=5),ncol = 1))+
  theme(legend.position = "none")
ggsave("../../out/image/UMAP_label_transfer_refWMASTRO_queryWMCX_robust_inverse_no_lab.pdf",width = 5,height = 4)
