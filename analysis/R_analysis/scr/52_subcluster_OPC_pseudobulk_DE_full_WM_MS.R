...
identical(rownames(counts_OPC_Ctrl),rownames(counts_OPC_NAWM))
# identical(rownames(counts_OPC_Mye),rownames(counts_OPC_NAWM))

# merge the three tables
# filter only for the CX samples
counts_OPC <- bind_cols(counts_OPC_Ctrl,counts_OPC_NAWM) %>%
  select(contains("WM_"))
...
saveRDS(counts_OPC,file = "../../out/object/counts_OPC_full_WM.rds")
saveRDS(colData_OPC,file = "../../out/object/colData_OPC_full_WM.rds")
...
# save the disign
saveRDS(design_OPC,"../../out/object/design_OPC_full_WM.rds")
...
pdf("../../out/image/heatmap_SampleCluster_pseudobulk_OPC_full_WM.pdf",width = 10,height = 6)
set.seed(3)
...
plot_vsd_OPC$data %>%
  ...
  scale_x_continuous(expand = expansion(mult = 0.1))+
  theme_bw() + ylab(plot_vsd_OPC$labels[1]) + xlab(plot_vsd_OPC$labels[2])
ggsave("../../out/image/PCA_pseudobulk_OPC_full_WM.pdf",width = 6,height = 4)
...
pdf("../../out/image/heatmap_cellType_pseudobulk_OPC_full_WM.pdf",width = 7,height = 5)
...
pdf("../../out/image/heatmap_cellType_pseudobulk_OPC_full_WM2.pdf",width = 7,height = 5)
...
saveRDS(ddsHTSeq_OPC_filter,"../../out/object/ddsHTSeq_pseudobulk_OPC_full_WM.rds")
...
# save the table
df_res %>%
  write_tsv("../../out/table/DE_pseudobulk_OPC_full_WM.tsv")
...
df_res_shr <-
  list(OPC.WM.MS_shr = res_OPC.MS_shr) %>%
  lapply(function(x){
    df <- x %>%
      data.frame() %>%
      rownames_to_column(var = "symbol") %>%
...
# save the table
df_res_shr %>%
  write_tsv("../../out/table/DE_pseudobulk_OPC_full_WM_shr.tsv")
...
ggsave("../../out/image/histogram_pvalue_pseudobulk_OPC_full_WM.pdf",width = 6,height = 4)
...
plot_volcano %>%
  ...
  ggrepel::geom_text_repel(
    data = plot_volcano[plot_volcano$col==1,] %>% group_by(conditionVsCTRL) %>% arrange(padj) %>% dplyr::slice(1:60),
    aes(label = symbol),segment.alpha=0.4) +
  facet_wrap(~conditionVsCTRL)+
  theme_bw()+
  theme(strip.background = element_blank())+
  theme(legend.position = "none")
ggsave("../../out/image/vulcano_plot_pseudobulk_OPC_full_WM.pdf",width = 10,height = 10)
...
pdf("../../out/image/heatmap_DEG_plot_pseudobulk_OPC_full_WM_shr.pdf",width = 7,height = 14)
...
pdf("../../out/image/heatmap_DEG_plot_pseudobulk_OPC_full_WM.pdf",width = 7,height = 14)
draw(ht2_shr_OPC2,heatmap_legend_side = "left",annotation_legend_side = "left",padding = unit(c(30,2,2, 2), "mm"))
dev.off()
