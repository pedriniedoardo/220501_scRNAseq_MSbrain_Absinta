# AIM ---------------------------------------------------------------------
# compare the subclustering with the original object
...
sobj_MG <- readRDS("../../out/object/129_MG_subcluster_HarmonySample.rds")
DimPlot(sobj_MG,group.by = "RNA_snn_res.0.9",raster = T)

# wrangling ---------------------------------------------------------------
# add the rename cluster Id from the res 0.9 to the original object
meta_test_subcluster <- sobj_MG@meta.data %>%
  rownames_to_column("barcode") %>%
  dplyr::select(clu_MG_sub = RNA_snn_res.0.9,barcode)
...
sobj_tot@meta.data$test_cluster <- sobj_tot@meta.data %>%
  ...
  mutate(test_cluster = case_when(is.na(clu_MG_sub) ~ expertAnno.l1,
...
test_long_run03_alt <- df_test_run03 %>%
  # force the order
  mutate(id = factor(id,levels = c("IMM","LYM","INH NEU","EXC NEU","AST","OLIGO","OPC","VAS","EPENDYMA"))) %>% 
...
#
test_long_subMG <- DotPlot(sobj_MG,
                           features = shortlist_features_list_long,
                           dot.scale = 8,
                           cluster.idents = T,
                           group.by = "RNA_snn_res.0.9") +
  RotatedAxis() +
  labs(title = "IMM and LYM subset res 0.9")+
  theme(strip.text = element_text(angle = 90))

# generate the ggplot object
# force the order suggested by matrina
df_test_subMG <- lapply(shortlist_features_list_long,function(x){
  test_long_subMG$data %>% 
    filter(features.plot %in% x)
}) %>% 
  bind_rows(.id = "cell_type")

# plot mapping radius
test_long_subMG_alt <- df_test_subMG %>%
  # force the order
  # mutate(id = factor(id,levels = c("IMM","LYM","INH NEU","EXC NEU","AST","OLIGO","OPC","VAS","EPENDYMA"))) %>% 
  mutate(cell_type = factor(cell_type,levels = c("IMMUNE","B_CELLS","T_CELLS","NEURONS","ASTRO","OLIGOLINEAGE","ENDO","PERICYTE","SCHWANN","EPENDYMA","STROMAL"))) %>% 
  ggplot(aes(x = features.plot,y = id)) +
...
test_long_run032 <- DotPlot(sobj_tot,
  ...
  RotatedAxis() +
  labs(title = "WM+CX with IMM and LYM subcluster")+
  theme(strip.text = element_text(angle = 90))
...
test_long_run032_alt <- df_test_run032 %>%
  # force the order
  mutate(id = factor(id,levels = c(0:19,"INH NEU","EXC NEU","AST","OLIGO","OPC","VAS","EPENDYMA"))) %>% 
  mutate(cell_type = factor(cell_type,levels = c("IMMUNE","B_CELLS","T_CELLS","NEURONS","ASTRO","OLIGOLINEAGE","ENDO","PERICYTE","SCHWANN","EPENDYMA","STROMAL"))) %>% 
  ggplot(aes(x = features.plot,y = id)) +
  geom_point(aes(size = pct.exp, col = avg.exp.scaled))+
  scale_radius(range = c(0, 8)) +
...
ggsave(plot = (test_long_run03_alt / test_long_subMG_alt / test_long_run032_alt),"../../out/image/129_DotplotLong_res0.9_MG_subcluster_test.pdf",width = 20,height = 18)
