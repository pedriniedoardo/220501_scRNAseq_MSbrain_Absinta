# AIM ---------------------------------------------------------------------
# with respect to the original analysis, the modifications are:
# set mito threshold at 5%
# remove sample 7 (s56)
# remove all the cell from cluster 7, which has been labelled from initial analysis as low quality cells.
...
id_sample <- dir("../../data/SoupX_default/") %>% 
  str_subset(pattern = "MA8483-7_all",negate = T)
...
list_datasc <- lapply(id_sample,function(x){
  ...
  datasc$test <- datasc@meta.data %>%
    mutate(test = percent.mt < 5 & nFeature_RNA > 500 & nFeature_RNA < 7000) %>%
    pull(test)
...
# laod the whitelist from the previus analysis
list_whitelist <- read_tsv("../../out/table/meta_data.combined_fix_filter_norm_doublet_integrated_SoupX_harmony.tsv") %>%
  separate(rowname,into = c("barcode","sample_id"),sep = "_") %>% 
  dplyr::select(barcode,seurat_clusters,orig.ident.cca) %>% 
  dplyr::filter(seurat_clusters!=7) %>% 
  split(f = .$orig.ident.cca)

id_objects <- names(list_datasc)
# x <- id_objects[1]
# fitler the both test and whitelist
list_datasc_fixed <- lapply(id_objects,function(x){
  test <- list_datasc[[x]]
  whitelist <- list_whitelist[[x]]
  
  # add the barcodes in the metadata
  test$barcode <- colnames(test)
  
  # filter for the whitelist
  datasc_filter1 <- subset(test, subset = barcode%in%whitelist$barcode)
  # filter for quality
  datasc_filter2 <- subset(datasc_filter1, subset = test == 1)
  
  return(datasc_filter2)
}) %>% 
  setNames(id_objects)
...
colnames(list_datasc_fixed_norm_doublet[[1]]@meta.data)
#
lapply(list_datasc_fixed_norm_doublet, function(x){
  meta <- x@meta.data
  colnames(meta) <- c("orig.ident","nCount_RNA","nFeature_RNA","percent.mt","percent.ribo","mt_bin","official_id","facility","origin","patient","pathology","disease","sex","age","PMI","year_of_extraction","test","not_outlier","barcode_whitelist","RNA_snn_res.0.8","seurat_clusters","pANN","DF")
  x@meta.data <- meta
  head(x@meta.data)
  dim(x@meta.data)[1]
})
...
pmap(list(names(list_datasc_fixed_norm_doublet),list_datasc_fixed_norm_doublet),function(x,y){
  ...
  # head(y@meta.data)
  #
  saveRDS(object = y,file = paste0("../../out/object/ManualClean/datasc_fix_filter_norm_doublet_",x,"_SoupX_ManualClean.rds"))
})
...
# save the total meta
meta_total_doublet %>%
  write_tsv("../../out/table/ManualClean/meta_datasc_fix_filter_norm_doublet_SoupX_ManualClean.tsv")
...
pmap(list(names(list_datasc_fixed_norm_doublet),list_datasc_fixed_norm_doublet),function(x,y){
  ...
  saveRDS(object = datasc_filter,file = paste0("../../out/object/ManualClean/datasc_fix_filter_norm_doublet_Singlet_",x,"_SoupX_ManualClean.rds"))
})
