...
# I am running the analysis on the all the cells 
...
list_data2 <- lapply(names(list_sig),function(x){
  ...
  df_meta %>% 
    write_tsv(paste0("../../out/table/modules_custom/144_Module_score_",x,".tsv"))
  ...
  ggsave(paste0("../../out/image/modules_custom/144_UMAP_score_",x,".pdf"),width = 13,height = 4)
  ggsave(paste0("../../out/image/modules_custom/144_UMAP_score_",x,".png"),width = 13,height = 4)
  ...
  ggsave(paste0("../../out/image/modules_custom/144_dist_score_ridges_",x,".pdf"),width = 10,height =9)
  ...
  ggsave(paste0("../../out/image/modules_custom/144_violin_score_",x,".pdf"),width = 9,height = 9)
  ...
  # scale_color_gradientn(colours = viridis::turbo(10))
  ggsave(paste0("../../out/image/modules_custom/144_UMAP_score_",x,"_tailored1.pdf"),width = 9,height = 8)
  ggsave(paste0("../../out/image/modules_custom/144_UMAP_score_",x,"_tailored1.png"),width = 9,height = 8,bg="white")
  ...
  ggsave(paste0("../../out/image/modules_custom/144_UMAP_score_",x,"_tailored2.pdf"),width = 9,height = 8)
  ggsave(paste0("../../out/image/modules_custom/144_UMAP_score_",x,"_tailored2.png"),width = 9,height = 8,bg="white")
...
list_plot <- lapply(list_data2,function(x){
  ...
  data2_avg <- x %>% group_by(expertAnno.l1) %>% dplyr::select(UMAP_1, UMAP_2) %>% summarize_all(mean)
...
ggsave("../../out/image/144_UMAP_custom_eliana_all.pdf",height = 16,width = 17)
ggsave("../../out/image/144_UMAP_custom_eliana_all.png",width = 17,height = 16,bg="white")

# load the module score for all the custom signatures
file_sig <- dir("../../out/table/modules_custom")

df_modules <- lapply(file_sig,function(x){
  df_modules <- read_tsv(paste0("../../out/table/modules_custom/",x)) %>%
    mutate(test = paste0(origin,"_",disease))
  return(df_modules)
}) %>%
  bind_rows()
...
df_modules %>%
  ...
  ggridges::geom_density_ridges(alpha=0.5)+
  facet_wrap(signature~expertAnno.l1,scales = "free_x")+
  theme_bw()+
  theme(strip.background = element_blank(), 
        panel.border = element_rect(colour = "black", fill = NA)) +
  # add the threshold for the specific cell type
  geom_vline(data = df_90 %>% filter(expertAnno.l1 %in% c("VAS")), aes(xintercept=thr),linetype = "dashed",col="red")
# scale_fill_manual(values = c("green","yellow","red"))
ggsave(paste0("../../out/image/144_dist_score_ridges_eliana_VAS_tailored.pdf"),width = 8,height =8)
