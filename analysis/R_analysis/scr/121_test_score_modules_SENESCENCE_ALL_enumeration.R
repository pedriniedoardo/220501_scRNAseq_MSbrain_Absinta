...
# save the table
df_en_090_final %>% 
  write_tsv("../../out/table/revision/modules_SENESCENCE/121_enumeration_ALL_090_threshold_MSStatus_refCX_cellID_senescence.tsv")
...
df_en_090_final %>%
  ...
  theme(strip.background = element_blank(),
        ...
        strip.text.y.right = element_text(angle = 0))+
  scale_fill_gradientn(colours = viridis::turbo(10),trans = "log",breaks = 4^seq(1,5))+geom_vline(xintercept = 0,linetype="dashed",col="gray")
ggsave(paste0("../../out/image/revision/modules_SENESCENCE/121_enumeration_log2Prop_ALL_090_threshold_above5_MSStatus_CXref_cellID_small2.pdf"),width = 18,height =8)
...
# make the call if the cell is senescent based on the distribution score of the senmayo
...
# make it a full dataset
enumeration_cellID <- bind_rows(list_enumeration_cellID)

# fill with 0 for the combination without senescent cells
enumeration_cellID_ref <- crossing(id = unique(paste(enumeration_cellID$signature,enumeration_cellID$orig.ident,enumeration_cellID$disease,enumeration_cellID$expertAnno.l1,enumeration_cellID$pathology_class,enumeration_cellID$origin,sep = "|")),
                                   sen = unique(enumeration_cellID$sen)) %>%
  separate(id,into = c("signature","orig.ident","disease","expertAnno.l1","pathology_class","origin"),sep = "\\|")

enumeration_cellID_wzero <- enumeration_cellID %>%
  group_by(signature,pathology_class,disease,origin,orig.ident,expertAnno.l1,sen) %>%
  summarise(n = sum(n))

enumeration_cellID_full <- left_join(enumeration_cellID_ref,enumeration_cellID_wzero,by = c("signature","orig.ident","disease","expertAnno.l1","pathology_class","origin","sen")) %>%
  # fill the NA with 0
  mutate(n = case_when(is.na(n)~0,
                       T~n)) |> 
  group_by(signature,pathology_class,orig.ident,disease,expertAnno.l1) %>%
  mutate(tot = sum(n)) %>%
  ungroup() %>%
  mutate(prop = n/tot) %>%
  # mutate(pathology_class = factor(pathology_class,levels = c("CX_Ctrl","CX_Demye","CX_Mye","WM_Ctrl","WM_NAWM","WM_CI","WM_CA","WM_Core"))) %>%
  # mutate(signature ="senmayo") %>%
  mutate(pathology_class = factor(pathology_class,levels = c("CX_Ctrl","CX_Demye","CX_Mye","WM_Ctrl","WM_NAWM","WM_CI","WM_CA","WM_Core"))) %>%
  ungroup()

# save the table
enumeration_cellID_full %>% 
  write_tsv("../../out/table/revision/modules_SENESCENCE/121_enumeration_ALL_090_threshold_MSStatus_refCX_cellID_sampleWise_senescence.tsv")

# # save a specific matadata table
# df_90_cellID2 <- df_modules_cellID %>% 
...
# 
# test_summary_cellID %>%
#   write_tsv("../../out/table/121_meta_SENMAYO_WM_CX_harmonySkipIntegration_AllSoupX_test.tsv")
...
ggsave(paste0("../../out/image/revision/modules_SENESCENCE/121_enumeration_log2Prop_ALL_090_threshold_above5_MSStatus_CXref_cellID_sampleWise_small3.pdf"),width = 18,height =8)

# export the table with proportions with all metadata
df_share <- enumeration_cellID_full %>%
  # filter(expertAnno.l1 %in% c(3,5,11)) %>%
  filter(sen == 1) %>%
...
df_share %>%
  write_tsv("../../out/table/revision/121_SENMAYO_prop_table.tsv")  
