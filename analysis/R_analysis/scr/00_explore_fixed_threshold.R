...
meta_total %>%
  ...
  ggplot(aes(x=official_id,y=value)) +
  geom_violin() +
...
meta_total %>%
  ...
  facet_wrap(official_id~var,scales = "free") +
  theme_bw() +
  scale_x_log10() +
...
meta_total %>%
  ...
  theme_bw() +
  scale_x_log10() +
  geom_vline(xintercept = c(500,7000),col="red",linetype="dashed") +
  annotate("rect", xmin=500, xmax=7000, ymin=0, ymax=Inf, alpha=0.1, fill="red")+
  theme(strip.background = element_blank())
# save the plot
