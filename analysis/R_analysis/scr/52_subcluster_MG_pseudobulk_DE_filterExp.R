...
saveRDS(counts_MG,file = "../../out/object/counts_MG.rds")
saveRDS(colData_MG,file = "../../out/object/colData_MG.rds")
...
# save the disign
saveRDS(design_MG,"../../out/object/design_MG_ctrl_refCX.rds")
...
pdf("../../out/image/heatmap_SampleCluster_pseudobulk_MG_ctrl_refCX.pdf",width = 10,height = 6)
set.seed(1)
...
ggsave("../../out/image/PCA_pseudobulk_MG_ctrl_refCX.pdf",width = 6,height = 4)
...
plot_vsd_MG$data %>%
  ...
  scale_x_continuous(expand = expansion(mult = 0.1))+
  theme_bw() + ylab(plot_vsd_MG$labels[1]) + xlab(plot_vsd_MG$labels[2])
ggsave("../../out/image/PCA_pseudobulk_MG_ctrl_refCX2.pdf",width = 6,height = 4)
...
pdf("../../out/image/heatmap_cellType_pseudobulk_MG_ctrl_refCX.pdf",width = 7,height = 5)
...
saveRDS(ddsHTSeq_MG_filter,"../../out/object/ddsHTSeq_pseudobulk_MG_ctrl_refCX.rds")
...
# save the table
df_res %>%
  write_tsv("../../out/table/DE_pseudobulk_MG_ctrl_refCX.tsv")
...
# save the table
df_res_shr %>%
  write_tsv("../../out/table/DE_pseudobulk_MG_ctrl_refCX_shr.tsv")
...
ggsave("../../out/image/histogram_pvalue_pseudobulk_MG_ctrl_refCX.pdf",width = 6,height = 4)
...
ggsave("../../out/image/vulcano_plot_pseudobulk_MG_ctrl_refCX.pdf",width = 10,height = 10)
...
df_res %>%
  ...
  theme(strip.background = element_blank()) +
  theme(legend.position = "none")
ggsave("../../out/image/MA_plot_pseudobulk_MG_ctrl_refCX.pdf",width = 10,height = 10)
...
df_res_shr %>%
  ...
  theme(strip.background = element_blank()) +
  theme(legend.position = "none")
ggsave("../../out/image/MA_plot_pseudobulk_MG_ctrl_refCX_shr.pdf",width = 10,height = 10)
...
pdf("../../out/image/heatmap_DEG_plot_pseudobulk_MG_ctrl_refCX_shr.pdf",width = 7,height = 14)
...
# PLOT DISPERSION ---------------------------------------------------------
pdf("../../out/image/dispersion_plot_pseudobulk_MG_ctrl_refCX.pdf",width = 5,height = 5)
