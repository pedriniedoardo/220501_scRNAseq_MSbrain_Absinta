# AIM ---------------------------------------------------------------------
# try to run the subclustering of the OLIGO cluster from the total object.
# the idea is to see if we can pop out potentially low quality cells or contaminants
...
# scobj@meta.data %>%
#   filter(expertAnno.l1 %in% c("OLIG")) %>%
#   group_by(seurat_clusters) %>%
#   summarise(n = n())
# 
# DimPlot(scobj, group.by="seurat_clusters",raster = T,label = T)

# pull only the cluster of interest
test <- subset(scobj,subset = seurat_clusters %in% c(0,4))
DimPlot(test, group.by="seurat_clusters",raster = T,label = T)

# save the seurat_clusters column in the metadata to keep track of where the cell go\
test$seurat_clusters_refCXWM <- test$seurat_clusters
...
sobj_total <- CreateSeuratObject(counts = test@assays$RNA@counts,
                                 project = "test_OLIGO",
...
# DimPlot(sobj_total_h,group.by = "orig.ident",raster = T)
# DimPlot(sobj_total_h,group.by = "seurat_clusters_refCXWM",raster = T)
...
saveRDS(sobj_total_h,"../../out/object/revision/100_subcluster_OLIGO.rds")
