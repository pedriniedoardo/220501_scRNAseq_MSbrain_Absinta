...
identical(rownames(counts_ASTRO_Ctrl),rownames(counts_ASTRO_NAWM))
# identical(rownames(counts_ASTRO_Mye),rownames(counts_ASTRO_NAWM))

# merge the three tables
# filter only for the CX samples
counts_ASTRO <- bind_cols(counts_ASTRO_Ctrl,counts_ASTRO_NAWM) %>%
  select(contains("WM_"))
...
saveRDS(counts_ASTRO,file = "../../out/object/counts_ASTRO_full_WM.rds")
saveRDS(colData_ASTRO,file = "../../out/object/colData_ASTRO_full_WM.rds")
...
# save the disign
saveRDS(design_ASTRO,"../../out/object/design_ASTRO_full_WM.rds")
...
pdf("../../out/image/heatmap_SampleCluster_pseudobulk_ASTRO_full_WM.pdf",width = 10,height = 6)
set.seed(3)
...
plot_vsd_ASTRO$data %>%
  ...
  scale_x_continuous(expand = expansion(mult = 0.1))+
  theme_bw() + ylab(plot_vsd_ASTRO$labels[1]) + xlab(plot_vsd_ASTRO$labels[2])
ggsave("../../out/image/PCA_pseudobulk_ASTRO_full_WM.pdf",width = 6,height = 4)
...
pdf("../../out/image/heatmap_cellType_pseudobulk_ASTRO_full_WM.pdf",width = 7,height = 5)
...
pdf("../../out/image/heatmap_cellType_pseudobulk_ASTRO_full_WM2.pdf",width = 7,height = 5)
...
saveRDS(ddsHTSeq_ASTRO_filter,"../../out/object/ddsHTSeq_pseudobulk_ASTRO_full_WM.rds")
...
# save the table
df_res %>%
  write_tsv("../../out/table/DE_pseudobulk_ASTRO_full_WM.tsv")
...
df_res_shr <-
  list(ASTRO.WM.MS_shr = res_ASTRO.MS_shr) %>%
  lapply(function(x){
    df <- x %>%
      data.frame() %>%
      rownames_to_column(var = "symbol") %>%
...
# save the table
df_res_shr %>%
  write_tsv("../../out/table/DE_pseudobulk_ASTRO_full_WM_shr.tsv")
...
ggsave("../../out/image/histogram_pvalue_pseudobulk_ASTRO_full_WM.pdf",width = 6,height = 4)
...
ggsave("../../out/image/vulcano_plot_pseudobulk_ASTRO_full_WM.pdf",width = 10,height = 10)
...
pdf("../../out/image/heatmap_DEG_plot_pseudobulk_ASTRO_full_WM_shr.pdf",width = 7,height = 14)
...
pdf("../../out/image/heatmap_DEG_plot_pseudobulk_ASTRO_full_WM.pdf",width = 7,height = 14)
draw(ht2_shr_ASTRO2,heatmap_legend_side = "left",annotation_legend_side = "left",padding = unit(c(30,2,2, 2), "mm"))
dev.off()
