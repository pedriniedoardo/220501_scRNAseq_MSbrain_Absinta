...
sobj_filter <- readRDS("../../out/object/revision/114_sobj_filter.rds")

DimPlot(sobj_filter,raster = T,group.by = "RNA_snn_res.0.3",label = T,split.by = "origin")
...
sobj_total <- sobj_total %>%
  ...
  ScaleData(vars.to.regress = c("nCount_RNA","S.Score","G2M.Score","origin","facility"), verbose = T) %>% 
...
saveRDS(sobj_total_h,"../../out/object/revision/115_WMCX_ManualClean3_harmonySkipIntegration_AllSoupX_4000_test.rds")
