...
enumeration_cellID_ref <- crossing(id = unique(paste(meta_full$orig.ident,meta_full$expertAnno.l1,meta_full$pathology_class,meta_full$origin,sep = "|")),
                                   SENEQUANTILE = unique(meta_full$SENEQUANTILE)) %>%
  separate(id,into = c("orig.ident","expertAnno.l1","pathology_class","origin"),sep = "\\|")

enumeration_cellID <- meta_full %>%
  group_by(pathology_class,origin,orig.ident,expertAnno.l1,SENEQUANTILE) %>%
  summarise(n = n())

enumeration_cellID_full <- left_join(enumeration_cellID_ref,enumeration_cellID,by = c("orig.ident","expertAnno.l1","pathology_class","origin","SENEQUANTILE")) %>%
  # fill the NA with 0
  mutate(n = case_when(is.na(n)~0,
                       T~n)) |> 
  group_by(pathology_class,orig.ident,expertAnno.l1) %>%
  mutate(tot = sum(n)) %>%
  ungroup() %>%
  mutate(prop = n/tot) %>%
...
ggsave(paste0("../../out/image/revision/modules_SENESCENCE/124_enumeration_SIT_WM_CX_harmonySkipIntegAllSoupX_expertAnno_zero.pdf"),width = 9,height =8)
...
df_share %>%
  write_tsv("../../out/table/revision/124_SIT_prop_table.tsv")  
