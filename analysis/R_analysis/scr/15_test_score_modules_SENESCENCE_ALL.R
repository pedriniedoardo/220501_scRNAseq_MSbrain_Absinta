...
lapply(names(list_sig),function(x){
  ...
  df_meta %>% 
    write_tsv(paste0("../../out/table/ManualClean/modules_SENESCENCE/Module_score_",x,".tsv"))
  ...
  data2 %>%
    ...
    # scale_color_gradientn(colours = c("blue","gray","red"))
    scale_color_gradientn(colours = viridis::turbo(10))
  ggsave(paste0("../../out/image/ManualClean/modules_SENESCENCE/UMAP_score_",x,".pdf"),width = 9,height = 3)
  ggsave(paste0("../../out/image/ManualClean/modules_SENESCENCE/UMAP_score_",x,".png"),width = 9,height = 3)
  ...
  data2 %>%
    ...
    ggplot() + geom_point(aes(x = UMAP_1, y = UMAP_2,col = signature_score1),alpha = 0.5,size = 0.2) +
    geom_text_repel(data = data2_avg,aes(x = UMAP_1, y = UMAP_2,label = seurat_clusters)) +
    facet_wrap(pathology_class~official_id) + theme_bw() + 
    theme(strip.background = element_blank()) +
    # scale_color_gradientn(colours = c("blue","gray","red"))
    scale_color_gradientn(colours = viridis::turbo(10))
  ggsave(paste0("../../out/image/ManualClean/modules_SENESCENCE/UMAP_score_split_",x,".pdf"),width = 13,height = 9)
  ggsave(paste0("../../out/image/ManualClean/modules_SENESCENCE/UMAP_score_split_",x,".png"),width = 13,height = 9)
  ...
  ggsave(paste0("../../out/image/ManualClean/modules_SENESCENCE/dist_score_",x,".pdf"),width = 12,height =9)
  ...
  data2 %>%
    ...
    arrange(signature_score1) %>%
    # mutate(gene = "Ptx3") %>%
    mutate(name_sample = paste0(pathology_class,"_",official_id)) %>% 
  ...
  ggsave(paste0("../../out/image/ManualClean/modules_SENESCENCE/violin_score_split_",x,".pdf"),width = 25,height = 9)
  ...
  data2 %>%
    ...
    scale_fill_manual(values = c("green","yellow","red"))+
    theme(strip.background = element_blank(),axis.text.x = element_text(hjust = 1,angle = 90))
  ggsave(paste0("../../out/image/ManualClean/modules_SENESCENCE/violin_score_",x,".pdf"),width = 9,height = 9)
})
